...
- **【リリス / MacBook 2026-09-03】🔴SP見切れの原因を特定。★まだ直していない（測れていない幅が残る）**
  - **★Chromeはビューポートを500px未満にできない**（320/360/390→500・768→768・1440→1440 を実測）
    → `review/*-sp.png` は「500pxで組んだページを390pxの窓で切り取った画像」。**見切れは撮り方由来の疑い**
  - **500px以上の全幅で横あふれ無し**（3ページ×5幅）。**PC幅1440の承認と再現度0pxは有効**
  - **★390px以下の実レイアウトはまだ測れていない。「直った」とは言えない**
  - 横あふれ検査の道具は作った（`tools/lsu_overflow.py`・自己テスト済み）。20本へ通せる
  - ★`--dump-dom` はこの環境で返らない → 計測値は画面に帯で描いて撮り、画像から読み戻す形にした
  - **★同じ対象に手をつけないでください**: `~/lifestandup-wp/` ／ v2.1フォルダ
...
- **【フランキー / MacBook 2026-09-03】LIFE STAND UP 写真の写り込みマスク＋割り当て更新 ── 走行中**
  - **読むだけ**: Drive 共有ドライブ `15-000_施設運営部門/LIFE STAND UP/WEBサイトRENEWAL/STAND UP Photo_260710`
    **元ファイルは1バイトも書き換えない・移動しない・外部へ上げない**
  - **書く**: `~/lifestandup-photo-plan/`（masked/ ＝ぼかし加工版・contact/ ＝確認用シート。ローカルのみ）
  - **★触らない**: `~/lifestandup-wp/`（リリス実装中・ステラ検査中）／本番サーバ／台帳・Notion・kintone
  - 有璽氏の可否回答：児童の顔✅同意済（掲載前に再確認あり）／スタッフの顔✅／有璽氏本人❌除外／
    送迎は運転手以外も／個人情報の写り込みは**捨てずにぼかして使う**
  - ✅ ぼかし12枚完了（`masked/` 加工版＋`compare/` 前後比較）。**12枚とも前後比較で当たりを検算済み**
  - ★新発見：**体操服の背中に学校名（KAWARAJYO OSAKA）**＝通学先が特定される。板書の児童名と同じ扱い
  - ★OCRはこの環境で使えない（tesseract無し／Swift SDK不整合／pyobjc ビルド失敗）＝目視で確認した
  - ✅ ぼかしは有璽氏承認済み（「このまま使って問題ない」）。**masked/ は作り直さない**
  - ✅ 確認シート完成 `確認シート_LIFE_STAND_UP掲載写真.pdf`（A4横7ページ・○×記入欄つき）
  - ★自分で見つけた誤り**4件**を修正：Photo-190→184／Photo-49→52／Photo-189→188／Photo-85→248
    **すべて同型＝縮小版で選び、写り込みだけ大きく見て、枠との適合を大きく見ていなかった**
  - ✅ 32枚すべてを確認シート上で目視検算済み（p1〜p6・6ページ全て）
...
- **【ビビ→ナミ→つる / MacBook 2026-09-03】予実の設計 ── ★器は完成。有璽氏の材料待ち**
  ...
  - **★有璽氏待ちは1件だけ**: 2026-08 の明細を `月次明細/2026-08/` へ（そのままの形で・8月をパイロット）
