...
- **`08_関係フォロー` の設計** … 列設計を提示・**承認待ち（まだ何も作っていない）**
  - 案件化前の相手（パートナー／紹介元／1on1／SalesBreaker反応者）の
    「次回アクション＋期日」を持つ層。案件シートにも活動ログにも置き場が無かった
  - ワークブック内に別タブで新設（2026-08-13 有璽氏が置き場を承認）
  - **番号は新ルールに従って 08**（獲得リストの後・案件10より前）
