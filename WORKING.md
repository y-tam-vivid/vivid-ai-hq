...
- **`08_関係フォロー`** … シート本体と自動入力は**構築済み・稼働中**。
  残るは仕上げの ㉗**v3** のみ（**ドライラン済み・バックアップ済み・本実行の承認待ち→実行依頼済み**）
  - 済: 14列のシート新設（㉕）／`onRelationEdit` 設置・動作確認（㉖）
  - 残: ㉗v3 ＝ ①09の90行目に⑤ブロック追記 ②ゴースト行の条件付き書式（既存0件）
    ③95の58行目から運用ルール3行 ④09の**A83のみ**「フォーム営業」→「問い合わせ営業」
  - Drive ㉗v3 `19--bfZJ-uD2BaPBuY7Vhe67C3JD6EyMFXpmJvUfKCu4`
  - **★v2 は使わない。** v1と識別子が衝突し、貼っても v1 が動いた（`toString()` で実測）。
    v3 は `_RV`→`_RW`・`addRelationViews*`→`relationFinish*` で名前空間ごと分離した
  - **プロジェクトに v1（`addRelationViews_`）が残っている。** 動作には影響しないが
    誰でも押せる状態。v3 は実行のたびに残存を警告する。落ち着いたら消す
