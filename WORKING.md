...
- **シート番号のルール** … 規則を提示・**適用は影響範囲の実測後**
  - 実測済み（2026-08-18）: 重複は `05`（問い合わせ営業リスト／テレアポリスト）と
    `97`（バックアップログ／同期ログ）。逆転は `91→20` と `40→09` の2箇所
  - **㊽で参照を洗う**（`1r7dPvVoh5MRXXKFLLUZGV8Qw39JGdp2hJxsyM-xaiaU`・実行待ち）
    `GLOBAL_SR = this` でランタイムの全関数を `.toString()` し、シート名の文字列を数える
  - **危険なのはGASだけ**。数式内の `'シート名'!A1` と入力規則は改名に自動追随する
  - ★㊽が見えるのは**このプロジェクトだけ**。別プロジェクトが参照していても出ない。
