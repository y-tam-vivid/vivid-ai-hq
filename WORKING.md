...
### 【ビビ着手中 2026-08-20 02:45】法人番号の一括補完（経路B）
...
### 【ビビ 2026-08-20 02:45】cron の投函口を作った（AIが自動実行に登録できない問題）

```
実測   Claude のセッションから crontab を書くと無応答のまま返らない
       （パイプ／ファイルの2経路・sandbox 有無の両方で同じ。プロセスはS状態で眠り続ける）
直し   bin/cron/<機械>.cron へ置く → bin/cron_apply.sh が不足行だけ入れる（追加のみ・冪等）
       vivid-sync.sh（cron */15）から呼ぶ ＝ **cron 自身に入れさせる**
番人   20秒で返らなければ諦めてログへ残す。同期本体は止めない（この面で実測済み）
```

- 投函済み1行 ＝ `00 8 * * * notion_customer_upsert.py --run --beat`（レジスタの予定08:00に合わせた）
- 入ったかの確認 ＝ `~/Library/Logs/vivid-cron-apply.log` と `crontab -l`
- **`notion_customer_upsert.py` は本実行まで済んでいる**（02:42）。台帳408社 ⇄ Notion486件を
  全件突合し**更新0・新規0**（＝既に同期済み）。ID充足 413/486。心拍もレジスタへ着弾
