...
- **`08_関係フォロー`** … **仕上げ（㉗v3）まで本実行済み。08は回る状態になった**（2026-08-15 07:50）
  ...
  - **★データ投入 完了（2026-08-18 14:49・突合OK全件一致）**
    ...
    - **★★自動化の前に直す穴（当方のコードの設計ミス）**
      既存行の更新で `I 最終接触日` に毎回 today を入れている。
      **毎朝走らせると最終接触日が毎日 today になり、
      「最終接触日＋声かけ周期」で入る期日が毎日1か月先へ逃げる**（95_運用ルール）。
      → **反応スコアが増えたときだけ最終接触日を更新する**へ直してから自動化する
