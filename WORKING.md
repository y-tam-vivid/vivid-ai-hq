...
- **【リリス / MacBook 2026-09-04】LIFE STAND UP スクリーンショット撮影(sp2) ── ✅完了**
  - deploy.py実行→配置OK（2経路一致）→ 新設 `make_sp2_images.py`（既存の tools/lsu_env.py・
    make_sp_images.py・make_review.py の部品を流用。新規の撮影/iframeロジックは書いていない）
    で `/stand-up/` と `/recruit/` を1440(直接)/390・360・320px(iframe経由)で撮影
  - 成果物 `~/lifestandup-wp/review/sp2/` 21枚。**6,500px超えは0件**（超えたら機械分割する設計だが
    今回は最大8,446px→6,000+2,446の2枚で収まった）
  - **実測（幅の一致）**: ファイル名の期待幅とPILで開いた実測幅を21枚全て突合 → 全一致
  - **theme/配下は触っていない**: 配下ファイルの最終更新が全て9/3（今回作業は9/4）で未変更を確認
  - **サーバ停止**: `tools/lsu_env.py` の scan_ports() で2経路（応答ポート・残プロセス）とも0件を確認
...
- **【ビビ→ナミ→つる / MacBook 2026-09-03】予実の設計 ── ★器は完成。有璽氏の材料待ち**
  ...
  - **★9/3 明細の第1便が到着**（5口座×2期間＝10CSV）。事業selectは11択（ライフスタディ追加済）
