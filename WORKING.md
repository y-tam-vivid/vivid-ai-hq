...
- **サービスアカウント導入** … 有璽氏が Google Cloud で作成中（手順は Notion⑥ に貼付）。
  鍵の置き場 `~/.vivid-relay/google_service_account.json`。
  **こちら側の準備は完了**（google-api-python-client / google-auth 導入済み・スクリプト待機）。
  鍵が来たら 疎通 → 全件調査 → 重複是正 → 移送v4 → cron まで一気に通す
- **台帳の重複** … C-0072（小川晃代）＝ B-0050（Blue Ocean Tradings合同会社）の代表者。
  01_顧客詳細にメール `akiyo@...` まで在った。**会社名1本でしか突合していなかったのが原因**。
  C-0073（福本）も同型の疑い。**是正は全件調査のあと**（読み書きの手段が入ってから）
