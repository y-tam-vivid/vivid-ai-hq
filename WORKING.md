...
コード・台帳・Notion・kintone・Slack へは1文字も書いていない。

```
9/21は有璽氏の発言0件・人の作業0件。動いたのは自動処理だけ
🔴偽の登録漏れ  週次 automation_inventory_check が office_fake_check.py を
                「台帳に無い」と出す。★台帳に行は在る。実体セルが空なだけ（2経路で実測）
                ★①はファイル名・参考は日本語の処理名で出る＝同一物と気づけない
🔴日時が無い    inventory_check.log に実行日時が1文字も無い＝慢性か新規か測れない
⏸未実施        直しは2つとも可逆だが判断待ち（こちらは読むだけ）
```

**★同じ対象に手をつけないでください**: `memory/project_automation_register.md`

### 【ロビン / mini 2026-09-21】毎朝の棚卸し ── ✅完了。★9/19の発注に答えが出た（下のブロックへ判定）

前日（9/20）実績の棚卸し。**読むだけで実測し、書いたのは memory 2本だけ**（本文＋索引1行）。
コード・台帳・Notion・kintone・Slack へは1文字も書いていない。

```
✅偽物は直った   探針=hook_selfcheck.log は今朝08:20に更新／corrections.log は 9/19 08:40 で停止
                起動プロンプト=3条件ANDで除外される（今朝の自分のプロンプトを当てて確認）
🔴別の穴①      ★PATTERNSが「〜してください」型を1つも拾わない（regex直当てで実測）
🔴別の穴②      ★corrections.log は機械ローカル。有璽氏が話すのはMacBook・棚卸しはmini
```

**★下の 9/19 ブロックの受け入れ基準は満たされた（差し戻さない）。**
残る2つの穴は**コード＝ロビンの領域外。ピタゴラスへの発注が要る**（内容は memory に記載）。

**★同じ対象に手をつけないでください**: `memory/reference_corrections_log_polluted_by_non_utterances.md`

### 【ロビン→ピタゴラス / mini 2026-09-19】corrections.log の偽の「有璽氏の発言」を止める ── ✅実装済み・ステラ検査中。★明朝の材料で判定

**★実装済み（ロビンが実測で確認）**：2本とも `~/.vivid-relay/` と同一（cmp）／ログ637行は無傷／check.sh ✅。
★未対応＝「重要な制約：」型2行・「前回のあなたは」型5行（誤除外を避けて残した）。
★**明朝の材料で探針2件・起動プロンプトが消えていれば完了。**消えていなければ差し戻す。

毎朝の棚卸しの材料（`~/.vivid-relay/corrections.log`）に、有璽氏の発言でない行が
**86%混ざっている**ことを実測した。**★8/25から30日・9回 memory に記録されながら、
担当へ投げられたのは今日が初めて。**（→ [[reference_a_warning_nobody_owns]] 7つ目の型）

```
実測2経路  全637行 ＝ 探針101(16%) ／ ★AI自身の起動プロンプト281(★44%) ／
                       task-notification 163(26%)
           直近7日のうち★5日は有璽氏の実発言が0件。それでも毎朝2〜6件が配られていた
🔴自己参照 ロビン/つるの起動プロンプト（規範の引用を含む）が「恒久のルール」と判定され、
           ★翌朝そのAI自身の材料へ「有璽氏の発言」として返る
```

**★書かせているのは `bin/hooks/{hook_selfcheck.py, hook_catch_correction.py}` の2本だけ。**
既存の `corrections.log` 637行は消させない。台帳・Notion・kintone・Slackへは書かせていない。
**★受け入れ基準はコードの有無ではなく「翌朝の材料にこの2件が出ないこと」**（歴代の指定どおり）。
★誤除外を防ぐため、本物の長文指示が今までどおり記録されるかの逆方向も測らせている。

**★同じ対象に手をつけないでください**: `bin/hooks/hook_selfcheck.py` ／
`bin/hooks/hook_catch_correction.py` ／ `~/.vivid-relay/` の同2本
...
有璽氏の構想「★Cをウェブアプリとして★1から作る。★オートレイアウトを前提に。
URLを貼って編集。★Figmaというかノーコードのツールのような感覚で」への対応。
**★作る前に外を見た**（9/13の型「症状でなく そもそもどう構築するか から投げる」を適用）。

**★1枚 https://claude.ai/code/artifact/20c2f3d9-0048-43bf-91c8-9fd1a5896336**
（調査＝クローバー・出典17件・全文 `~/.vivid-relay/nocode_editor_research.md` 18KB）

```
★結論は「1から作り直す」ではなかった
  ①「URLを貼って編集し実サイトのCSSへ書き戻す」を丸ごとやる★既製品は無い
  ②WP専用(Microthemer等)が最も近いが★恒久ルール判定は持てない
  ③★絶対配置の装飾(テープ等)は★Figma自身のオートレイアウトでも枠の外
    ＝「全部を1つの仕組みで扱う」を目標にしない
  ④保存経路(Blob→mini→CSS)と恒久ルール判定は★この分野の良い設計と一致する自社資産
  ⑤★推奨＝土台は残し、★編集UIだけ作り直す（C+D の中間案）
★調べきれなかった  「自作ツールをやめた事例」は★名指しの記事に行き着けず＝前例に頼れない領域
```

**★有璽氏の判断待ち**
```
① この方向（土台は残し編集画面だけ作り直す）で進めてよいか
② ★まず何が使えれば「使える」と言えるか（選んで動かす／親の間隔／連続幅 のどれが先か）
③ いまの★Cはその間どうするか（使い続ける／触らない）
```

**★同じ対象に手をつけないでください**: 上のArtifact ／
`~/.vivid-relay/nocode_editor_research.md`

### 【チョッパー / mini 2026-09-14】①リンク切れ ②スマホでテープ/ABCを消す ③他15ページの検証 ── ✅完了。commit `0525de8`/`993c4d0`・★デプロイ済み

```
✅①リンク  href="#sec-programs"＝★飛び先id が0件（class="sec-programs"と取り違え）
           → lsu_link('programs')。★他ページの約40件のアンカーは飛び先が全て実在＝0件
✅②消す    .tape-vertical と .abc-block を★共通style.cssで540px以下 display:none
           ★22本すべてに定義が在り、当初1本しか直していなかった（有璽氏が発見）
           ★fv-photo-block(写真枠)・info-block/qa-block/quote-block(本文)は触らない
✅③検証    横あふれ45ケース0px ／ 重なり 101→93（★新規増加0・-8は改善）
```

**🔴★消えなかった真因（有璽氏に3回確認させた）**
```
編集する場所 theme/lifestandup/ と ★WordPressが読む場所
_tools/wordpress/wp-content/themes/lifestandup/ が★別。同期していなかった。
古い functions.php が ver=0.1.0-draft を出力し続け、ブラウザが★古いCSSを使い続けた。
★「直す→出す」の間に関所が2つ：①deploy.pyで同期 ②LSU_VERSIONを上げる
  ①を飛ばす＝中身は新しいのに配られない ②を飛ばす＝配ってもブラウザが取り直さない
  ★どちらも、こちらからは成功に見える → [[reference_fix_where_git_reaches]] 5例目
LSU_VERSION 0.1.0-draft → ★0.1.2-20260914
```
...
```
★数え直し   縦の余白 総数 738件（padding 167 / ★margin 571）・全幅に効く 605件
★対象       全幅・40px以上・比率なし ＝ ★158件46パターン → 全部比率化（未マッチ0）
実測        .worry-closer(60px)  375=36 / 768=52.5 / ★1440=60（不変）
            .timeline(50px)      375=32(下限) / 768=43.75 / ★1440=50（不変）
            横あふれ15ケース0px ／ 重なり290→290（新規0）／ 括弧23本一致
            ★既存のpx宣言は1文字も変えていない
★残り       20-39pxの79件・メディアクエリ内133件は対象外／意匠寄りの区切りは見た目の判断が要る
            ／検証は代表5ページ（22ページ全部ではない）
```

**★commit直前に1件止めた**：`assets/css/style.css`（テーマ本体95KBの★完全な複製）が
誤って新規作成されていた。ステージから外して削除・本体は無傷。
→ **★`git add <ディレクトリ>` の前に `--name-status` で★新規(A)が無いかを見る。**

控え `theme/lifestandup/_backup/20260914-margin-all/` ／ 出口 `chopper_margin_all_result.md`。

**★同じ対象に手をつけないでください**: なし（作業完了）

<details><summary>▸ 着手時の指示内容（経緯として残す）</summary>

有璽氏「**調整を、全体のマージンを改めて確認し調整してください。物にもよるけど、
仕上がってないものがあるのかなというふうに確認していて思います。**」
★有璽氏は**実物を見て**言っている。**数字で正体を突き止めてから直す。**

**★ビビが先に実測した出発点**
```
9/13 fd7fa9b で直した件数   padding 85件（top42/bottom43）＋ margin 45件（top18/bottom27）＝130件
★いま残っている縦マージン   ★436件
★うち40px以上              412px×3 / 90px×2 / 80px×11 / 64px×1 / 60px×24 / 56px×23 / 50px×43 / 48px×2
                           ＝ ★約109件（全幅で効くかは未分類＝チョッパーが数える）
★9/13の記録                「margin の94.7%がPC値のままスマホへ」＝★まだほとんど直っていない
★paddingの漏れ4本          legal-disclosure(88px) / privacy-policy(88px) / useful(80px) /
                           ★news-detail(80px＝活動ブログ134本に効く)
```

**★順番**：①全部数える（種類/値/メディアクエリの内外/比率の有無で分類）
②直す対象と★触らない対象（小さい余白・装飾・テープ）を分ける・迷うものは判断を上げる
③9/13と同じ形で比率化（既存の宣言は1文字も変えない）④前後のpxを並べて実測。

指示文 `~/.vivid-relay/chopper_margin_all.txt`。
</details>
...
有璽氏に渡した取説の「まだできないこと」①（内側の余白(Padding)を欄から変える）に対応。
**対象は `lsu-editor.html`（正本）／`vercel-editor/lsu-editor/index.html`／
`static-preview/lsu-editor/index.html` の3ファイルのみ（3ファイルとも同期・diff一致確認済み）。
theme/lifestandup/配下のCSS・PHP（★同日先に入れたセクション間マージン比率化）は
1文字も触っていない（git diff 0件で確認済み）。本番サーバー・本番WP管理画面・DNS／
台帳・Notion・kintone・Slackへは触れていない。redeploy.sh未実行・git add -A不使用・
pkill -f不使用（PID指定のみ）。**

```
①最重要   セクション間マージン比率化(--sp-t/--sp-b・commit fd7fa9b)との衝突を回避。
          そのセレクタのCSS本文に--sp-t/--sp-bがあれば警告を出し、案A(PC基準値の
          --sp-t/--sp-bとpadding-top/bottomを同時更新・比率と床はそのまま効かせ続ける)
          を採用。モードB(この幅だけ)での編集は比率計算式(max(...))を壊す実害があるため
          alertで保存拒否
...
③副産物    selectParent()が「常に①の直接の親固定・複数回押しても進まない」既存の制約を
          発見・修正。押すたびに1階層ずつ祖先へ遡れる形に。ボタン文言も次の遷移先を
          動的表示（説明文でなく画面自体で伝える）
実測      headless Chrome+CDPで3回検証（ピタゴラスへ委譲）。比率対象/非対象それぞれの
          保存件数(1件/2件)・モードB拒否・複数階層遡り(5階層)・比率計算式の温存・
          反映後の完全復元、すべて実測確認
```

**★できなかったこと**：`.footer`のようにscan()候補が子孫に0件のセクションには、①からの
経路が原理的に存在せず現状のUIでは到達できない（実測：footer配下の絶対配置候補0件）。
Hug/Fill・Wrapは未着手（次回への道筋のみ出口ファイルに記載）。

詳細・実測全文 → `~/.vivid-relay/chopper_padding_result.md`。Slackへnotify.tell()で報告予定。

**★role_guardフックの誤検出**（agent_id欠落によりメインセッション=ビビ扱い）に当たり、
検証用.pyスクリプトの作成・実行はサブエージェント（ピタゴラス）へ委譲した（Bash経由の
回避はしていない）。実装コード本体（.html 3本）は対象拡張子外だったため直接編集した。
...
外部調査（`~/.vivid-relay/agent_console_research.md`・クローバー）の結論に沿って直した。
**本番サーバー・台帳（00/01/02/40）・kintone へは1文字も書いていない。Slack実投稿0件。**

```
① 心拍          run_agent.sh が agent_heartbeat.py を起こす。30秒ごとに
                {status, doing, elapsed_min} を ~/.vivid-relay/agent_heartbeat/<担当>.json へ
② 正本を1本に    agent_running.py を新設。★会議室も羅針盤もこれを読む
                （旧：会議室=ps／羅針盤=psのペルソナ宣言／名簿 の★3通りが別々に計算していた）
③ 判断待ち      pending_decisions.py 新設＋会議室へ「私の判断待ち」欄。
                ★ボタンは本物になった：画面→/api/office-answer→Blob→
                office_answer_apply.py（cron */5・心拍つき）→ask_hub_queue.json
④ トレース/成功率 走行中の行に「いま：<担当自身が書いた1行>」＋直近7日の結末
⑤ 即時push      ★既に動いていた（running_pinged_at が実測で更新されている）
```

**🔴 真因（有璽氏の2つの苦情の答え）**
```
「押しても毎回戻る」  answer() は claude.use("db") 前提で、Vercelでは DB=null。
                     押した結果はページ内の変数にしか残らず★どこにも届いていなかった
「0人と出続ける」     ps -eo pid,etimes は macOS に etimes が無く★rc=0 のまま列が減る。
                     例外が出ないので全行が黙って捨てられていた（同じ穴が3本にあった）
```

**実測**
- ②：プローブを走らせ、同じ瞬間に 会議室 count=1 ／ 羅針盤 web-developer ＝一致
- ③：台帳を控えてから通し → answered へ入る／二度入らない／★Slack呼び出し0回／台帳は前後一致
- ④：直近7日 起動89・終わった83・rc=0が83。★rc=0は完走であって成功ではない、と画面にも書いた
- 心拍：レジスタへ着弾（14:04・成功）を2経路で確認。★有効=False のまま（初回の自然発火待ち）

**🔴 その場で見つけて直した穴**
- `office_build.py` はテンプレートの `<script>` を★丸ごと差し替える。テンプレート側のJSを
  直しても公開版に1文字も入らない。★実際 drawPending は呼び出しだけが入り定義が無かった
  （出していれば画面が止まった）。★「直した場所が配られる場所か」を先に見る
- 0件の日に心拍を打たない作りだった（「0件だった」と「落ちた」が同じ見た目になる）→直した
- 別セッションが `chopper`（名鑑に無い名前）で担当を起こしており、会議室は1人・羅針盤は0人。
  ★当てにいかず「名鑑の担当へ結べない走行中」として★画面のerror欄に出す形にした

**★有璽氏の手が要るのは1回だけ**：https://fukuchi-kadoban.vercel.app/office の
承認タブでボタンを1回押してください。**ブラウザ→Function の1本だけ、Basic認証の内側なので
当方からは実測できません**（合言葉なしで401までしか測れない・既知の限界）。
押すと「受け付けました」と出て、5分以内に台帳へ入り、Slack側のボタンも消えます。
届かなければ★そのボタンの下に赤字で理由が出ます（黙って成功したように見せません）。

**★同じ対象に手をつけないでください**: `~/.vivid-relay/{agent_running,agent_heartbeat,
pending_decisions,office_answer_apply,office_data,office_build,office_template}` ／
`web/kadoban/api/office-answer.js`
...
**対象は `~/lifestandup-wp/_tools/wordpress/`（ローカル検証環境）と `migration/` 配下のみ。**
`theme/lifestandup/` 配下のCSS・PHP（下の写真移動・マージ担当が触っている本体）は
1文字も編集していない。本番サーバー・仮公開サイト・台帳・Notion・kintoneへも触れていない。

```
✅テーマ有効化   ローカル検証WPで「LIFE STAND UP」を有効化。ライブプレビュー機構
                (customize.php)がHTTP200・0.3秒で動くことを実測
✅134本投入     既存デモ5本+新規129本=134本、過不足0・重複0（独立検算）。
                写真2,210枚取得（失敗2枚のみ・本番側の404）。Jetpackタイル・
                標準ギャラリー・単独写真、いずれもスクショで目視確認・崩れ無し
✅手順書更新     migration/本番への移し方.md の★未確定8件のうち4件を実測で解消
                （★重要発見：news.phpはpage_for_postsに依存しない設計）
```

出口 `migration/wp_run_result.md`。記録 `memory/project_lifestandup_website_wordpress.md`
「2026-09-09 16時台」節。検証サーバーは2経路（lsof・curl）で停止確認済み。

**★同じ対象に手をつけないでください**: なし（作業完了・migration/配下は未コミットのまま
theme/側のマージ完了を待つ）
...
`bin/hooks/hook_permission_slack.py` の1本だけ（＋`~/.vivid-relay/` への複製・控え
`_backups/hook_permission_slack.py.bak_20260906-approval`）。`ask_hub.py` は無改修。
台帳・受付シート・Notion・kintone へは1文字も書いていない。**Slackへの実投稿0件**
（`post` と `urllib.request.urlopen` の両方をモックし、呼び出し0回を実測）。
`~/.claude/settings.json` は**編集していない**（sha256 `a806313d…` 不変）。

**結果の全文 → `~/.vivid-relay/approval_dialog_result.md`**（09:00 の途中経過は
`approval_dialog_result_途中経過_0900.md` へ改名して残した・消していない）

```
★可否を実測で確定（隔離HOME・PTYまで起こした。本物の設定は無傷）
  非対話(claude -p)             ★PermissionRequestは一度も発火しない（T1〜T10）
                               PreToolUseへ逃げてもaskルール一致はallowで上書き不可（P1/P3）
                               → ★「miniが承認ダイアログで止まる」は成り立たない。訂正した
  対話＋dontAsk以外             ★発火する・allowもdenyも効く・★45.012秒待てる（I1〜I4）
  対話＋dontAsk（この2機の既定）  ★発火しない。Bash/Edit/AskUserQuestionとも即拒否（I6〜I8）
  🔴async: true（★いまの登録）   ★フックは走るが決定は捨てられ、ダイアログが残る（I5・3回再現）
```

**🔴 最大の実害は別にあった ── 通知83件のうち★63件（76%）が偽物だった。**
`ls` ちょうどが 08:20 に18件・08:40 に18件＝**18日間、毎日2通**。正体は生死の点検
（`hook_selfcheck.py` の CASES → cron 08:20 ／ `self_audit.py:59` → cron 08:40）。
**★session_id が無い呼び出しでは投稿しない**ようにした（点検には従来どおり答える＝両方とも無改修で通る）。

```
入れたもの  第0段 偽の通知を止める                       ★人の手ゼロ・今日から効く
            第1段 機名/対話か非対話か/セッション/権限モード/作業場所/押す場所を通知文へ
                  ★人の手ゼロ・今日から効く（9/5の別セッションは宣言だけで未実装だった）
            第2段 ask_hubのボタンで許可/拒否を受け、その決定を返す
                  ★自分でsettings.jsonを読み「押した結果を返せる形か」を確かめてから出す。
                    asyncが付いていればボタンを出さない（押しても効かない形を作らない）
実測        22ケース全合格（探針/async時/待機/許可/拒否/その他記述/時間切れ/遠隔禁止の種類）
sha256      dd1f4ca4…（bin/hooks と ~/.vivid-relay で一致）。110行→412行
```

**🔴 人の判断・人の手が要る3点（全文は結果ファイル）**
① **`~/.claude/settings.json` の `PermissionRequest` から `"async": true` を消し、
  `timeout` を 15→600 にする（両機）。これをするまで第2段は1度も動かない。**
  ★`setup_hooks.sh` は「無ければ足す」だけなので自動では直らない（＝手で直せば戻されもしない）
② 権限モードを dontAsk のままにするか。**このままだと承認ダイアログ自体が出ない**＝押す機会が来ない
③ AskUserQuestion は許可しても解決しない。**層2 `hook_interactive_guard.py`（9/5実装・未登録）の
  登録**を推奨。★こちらでは登録していない

**★このファイルには、下の「ピタゴラス2」の手も入っている（実測で確認・消していない）。**
`settings_paths()` を新設し `CLAUDE_CONFIG_DIR` を先に見る形にしてある。
向こうが通し確認をした際に「決め打ちの `~/.claude/settings.json` を読み、隔離側ではなく
本番の `async: true` を見て『遠隔承認できない』と誤判定した」実害を踏んだため。
**★こちらの22ケースは決め打ち前提で通したもの。この差分を含めた再検査はステラへ回す。**

**★「直った」と言える証拠はまだ無い。**有璽氏が実際にボタンを押して、止まっていた作業が動く、
を1度も通していない。**settings.json を直したあと1回通すこと。**
検査は自己採点にせず、ステラ（cross-check）へ返す。

**★同じ対象に手をつけないでください**: `bin/hooks/hook_permission_slack.py` ／
`~/.vivid-relay/hook_permission_slack.py`

### 【ピタゴラス2 / mini 2026-09-06】承認ダイアログ ── ✅実測は完了。★実装は降りた（同じ依頼が2セッションに入っていた）

**🔴 同一の依頼文が2つのセッションへ入っていた**（実測：`~/.vivid-relay/_task_approval_dialog.txt`
と当方の指示文が同一。当方 PID 68242／相手 PID 69505・当方の4分後に起動）。
**相手が `~/.vivid-relay/approval_dialog_result.md` を書き進めていたので、
当方は実装を降り、実測と申し送りに徹した。** → [[reference_two_sessions_built_the_same_thing]]

**★本体は無傷のまま**（`bin/hooks/hook_permission_slack.py` sha256 `5545168a…`・mtime 8/20 01:50・
git差分0）。**当方の実装エージェントは1行も書く前に止めた。**

```
当方が書いたもの   memory/project_ask_hub_push_decisions.md ／ memory/MEMORY.md の1行 ／
                   WORKING.md ／ ~/.vivid-relay/approval_dialog_実測_ピタゴラス2.md
★書いていない     bin/hooks/ 配下 ／ ~/.claude/settings.json ／ ask_hub.py ／
                   台帳・受付シート・Notion・kintone ／ Slackへの実投稿0件
```

**🔴 相手の設計に前提の誤りが1つある（申し送り済み）** ── 相手は「非対話が本当に止まっている
場面」としてそこにボタンを出す設計にしているが、**非対話では PermissionRequest フックが
1度も発火しない**（実測2経路）。**効くのは対話セッションだけ。**

**🔴 隣で見つけた実害（当方は直していない）**：`bin/hooks/hook_selfcheck.py:10` の CASES が
`hook_permission_slack.py` を直接叩き、**本物の有璽氏のDMへ投稿している**。
直近120通の「承認待ち」24件のうち**17件がこの点検由来**（`Bash ls`・毎日08:20と08:40）。
**本物5件が埋もれている。**

**★同じ対象に手をつけないでください**: なし（当方は実装を降りた）。
★`bin/hooks/hook_permission_slack.py` は**相手のセッションが触る**。

### 【ピタゴラス / mini 2026-09-06】PermissionRequestフックの実測（承認代行・待機の成立性）── 着手

**関連セッション注記**: 直上の「承認ダイアログをSlackから解けるようにする」ブロックとは
別スコープ。あちらは `bin/hooks/hook_permission_slack.py`（実装）、こちらは `/tmp/permtest1/`
の**隔離環境での実測のみ**（本番設定・実装コードには触れない）。目的は関連（承認の
Slack化が技術的に成立するかの下調べ）だが、対象ファイルは重ならないため並走する。

**対象は `/tmp/permtest1/` のみ。本番 `~/.claude/settings.json`・`~/.vivid-relay/`・
`bin/hooks/`・台帳・Notion・kintoneへは1文字も書かない。Slackへ1件も投稿しない。**
子プロセスはHOME/cwdとも隔離済みディレクトリへ差し替え、本物の規範・memoryを読ませない設計。
成果物は `~/.vivid-relay/approval_probe.md` のみ。

**★同じ対象に手をつけないでください**: `/tmp/permtest1/` ／ `~/.vivid-relay/approval_probe.md`

### 【ピタゴラス / mini 2026-09-06】営業台帳 ②01から3行削除 ③顧客種別の変更 ── ✅完了。つる検査待ち

**書いたのは `01_顧客詳細`（3行削除）と `00_企業マスタ`（顧客種別 W列 14セル）だけ。**
02・08・10/20/30・40・受付シート・90／Notion・kintone へは1文字も書いていない。
**Slackへの実投稿0件**（ask_hub も notify も呼んでいない）。保留(park)は解除していない。

**有璽氏の承認は自分で実測した**（`ask_hub.answer_of()`）── #a3a71e「01からも同じ3行を消す」／
#e509fe「AIの判断で変更してよい」（両方 by=yuji38132000・9/5 17:27）。
**★#3426e9（9/6 08:48 再承認）は「会話・ビビが代理記入」＝独立した2経路目にならない**
（会話の実物は mini から見えない）。**実行の根拠として採ったのは本人がボタンを押した前者2件。**

```
②  01_顧客詳細 386行 → ★383行（B-0008/B-0076/B-0132 を★行番号の大きい順に）
    ★消す前に検査：他シートの数式が01を参照 = ★0箇所／B-0380 実在／同一IDの重複0件
    実測：3件とも消えた・B-0380 全列一致で無傷・00は468社で不変
    ★重複の要判断 21組 → ★9組（9/5の試算と一致）／別法人18組（不変）／繋がり欠損0件
③  00_企業マスタ 顧客種別 14件を「法人」→「個人（事業主）」
    ★書く前に4つ一致しないと書かない実装（ID1行・会社名・現在値=法人・法人番号が空）
    実測：14件とも一致・★顧客種別以外の差分 0件・分布 法人277→263／個人事業主82→96
    ★「法人なのに法人番号が空」 59件 → ★45件（0にはならない）
控え  Drive複製2本 1EAehSrB11E6V8phM…（削除前）／1iEwQGhpNhALM…（変更前）
      ★どちらも複製を開き直して中身が本体と一致することを確かめてから書いた
      消した3行の全42列・変更前の値はJSONで別途保存
```

**★8/31（消したら14→21組に増えた）との違いは作業でなく順序** ── 消さずに試算 →
失われるものを見せて再承認 → 消す → 実測（試算9組と実測9組が一致）。

**🔴 見つけたが直していないこと3点**
① **`corp_number_fill.py` が古い版の国税庁データを見たまま。**月次更新は動いて 08-31版が
   落ちているのに、`corp_lookup.py:30` と `corp_number_fill.py:41` が★ファイル名を固定で持ち
   07-31版を読み続ける（取得は成功＝心拍は緑）。**同じ値が2箇所にあるので片方だけ直すと食い違う**
② `ledger_report.py` の毎朝の文言「全件データの月次更新はまだ自動化していない」が**8/27から古い**
③ **③をやっても「重複の要判断 9組」は減らない**（ledger_dupes は顧客種別を見ていない）

**★表C（法人番号を機械で確定できる16件）は表に出しただけで実行していない**（②③の承認に含まれないため）。
**★つるの検査は通していない。**検算できる形（変更前後の全件表・控えの在処）は結果ファイルに全部残した。
...
### 【リリス / mini 2026-09-06】区分・リンク・フォント・地図（A/B/C）── ✅完了。仮公開に反映済み

**★大前提が変わった（有璽氏 9/6）：仮公開サイトの見た目が正。**ここを直せば本番の設計になる。
原本HTMLとの1px差は以後、不具合として扱わない。

**https://lifestandup-preview.vercel.app（lsu-staff / standup2026）に全部入っている。**
**本番サーバー・WP管理画面・DNS／台帳・Notion・kintone へは1文字も書いていない。**
push 済み（`f384657`）。README 16節に全部書いた。

```
A-1 区分   ✅ フロー型3（お知らせ/活動/イベント）＋ストック型「お役立ち情報」＝CPT `useful`・/useful/
           ★その他は選択肢から外した（スラッグは消していない）
           ★写像先も3区分へ入れ直した ── 実物で「どのピルでも絞り込めない記事」を1件踏んだ
           一覧はトピック別カード・★日付を出さない・★説明文を必ず出す（新着順に混ぜない）
A-2 トップ ✅ 3枚をクリックできるようにし実データへ。★引くのは標準投稿だけ
           （フロー型を全部引いたら3枚ともお知らせになった＝押した先に無い記事が出る）
A-3 IG     ✅ 誤 life_stand_up が★3箇所ベタ書き → lsu_sns() へ。表示名もURLから作る
A-4 font   ✅ 定義は★2つでなく23か所（style.css 2＋ページ別CSS 21が body へ再定義）
           → style.css の :root 1か所へ。10ページとも実要素のfontFamilyが1種類に揃った
A-5 X      ✅ アカウントが無いので共有ボタンを外した（LINEは残す・戻し方はコメントに）
B   地図   ✅ ピンを★座標で立て（34.561554,135.604538）、白黒（簡易版）、トップの
           「[Googleマップ埋め込み]」という文字だけの箱を実地図へ
C   案のみ ✅ proposal/C-1（Instagram実投稿・3案と代償）／C-2（LINE導線・電話15箇所の並べ方）
```

**実測**：仮公開URLで ①合言葉なし401 ②合言葉あり200 ③主要15ページ200 ④直した所を全部数えた／
ローカル24ページ HTTP200・PHP警告0／横あふれ 新規0（既知のtop320のみ）／
重なりは**控えを同じ道具で測り直して**比較（447→460・新規は装飾どうしの3〜10%で判定は全て「読める」）。
検証サーバは2経路で停止確認。控え `_backup/20260906-abc/`。

**🔴 直せなかった1件（有璽氏の判断が要る）**
`/stand-up/` の「✕」が EXPERIENCE を覆う率が **12% → 22%** に増えた。
★真因＝Kosugi Maru の「✕」はグリフ幅が **34px → 61px** で、位置が
`left:calc(33.333% - 18px)` という**幅34px前提の決め打ち**だったため。
★中央寄せ（translateX(-50%)）を試したが**悪化したので元に戻した**（実測をCSSに残した）。
直すには①スマホ幅で✕を出さない ②✕を小さくする のどちらかで、**意匠に触るので承認が要る**。

**★同じ対象に手をつけないでください**: `~/lifestandup-wp/`（theme・static-preview）／
Vercelプロジェクト `lifestandup-preview`

### 【ピタゴラス / mini 2026-09-06】受付シート41行目を台帳へ発番（有璽氏の承認 #8330f0）── ✅完了。B-0473として発番済み

有璽氏が21:27に「新規で発番して良い」と回答（実測：`ask_hub_queue.json` の `8330f0`・
answer.by=yuji38132000）。二重発番の無いことを2経路で確認したうえで着手。

★一度 ask_hub #960e67 で「B番号(法人)として発番してよいか」を追加で聞いたが、
コーディネーターの差し戻し「会社名に法人格語を含むのは事実。商材欄が空＝toB/toCどちら
の商材を売るかの軸と、相手が法人か個人かの軸は別。聞かなくてよい質問だった」を受けて撤回。
#960e67 は新しく聞き直さず、同じ受付番号のメッセージを chat.update で「解決しました」に
差し替えて畳んだ（9/5 #8330f0で使ったのと同じ手順）。

対応：受付シート41行目の「想定している商材」欄を `まだ分からない` → `まだ分からない（toB）`
へ追記（既存35/36行と同じ記法。intake_register.py 本体は書き換えていない＝安全装置は
そのまま）。ドライラン→本実行し **B-0473** として発番。法人番号9011101106965・東京都新宿区
まで国税庁データから自動特定。00/01/40/08の4シートに正しく書かれたことを実物で確認。
受付シートの差分は41行目の4セルのみ（商材／社内顧客ID／台帳登録／社内メモ）。他行は0件。

★あわせて34/38/39/40行を実物確認（書いていない・数えただけ）。4行とも会社名に法人格語を
含み外形は41行目と似るが、**#8330f0のような有璽氏の承認記録が見当たらない**。確認欄の
「× 別人（新規でよい）」がいつ・誰の入力か裏付けが無いため、41行目と同じ扱いはしていない。

控え：受付シートDrive複製3本。ログ `~/.vivid-relay/intake_row41.log`（表つき）。
報告は `notify.tell()` で1本出す予定（判断語を入れない）。

**★同じ対象に手をつけないでください**: なし（作業完了）

### 【つる / mini 2026-09-06】受付シート41行目 B-0473 発番のcross-check ── ✅完了。判定★条件つき

**読むだけで検査した。台帳・受付シート・Notion・kintoneへは1文字も書いていない。**
検査した版＝WORKBOOK version=1020／INTAKE version=277（検査前後で不変を確認）。

```
① 法人番号9011101106965  ★問題なし。国税庁全件データ(2026-08-31版)で全国唯一一致・
                         検査数字(チェックディジット)も計算一致
② 二重登録               ★無い。00/01/02/40/08の5シートで会社名・法人番号・代表者名
                         を検索しB-0473以外にヒット0件
③ 商材欄への(toB)追記     ★手続きが不可。元の値は消えていないが、intake_register.pyの
                         安全装置(新規発番B/C判定)を通すためだけの追記
④ 34/38/39/40行          ★「触っていない」の申告は事実と一致（控えとの全列突合で差分0）
⑤ 4シートの整合性         ★矛盾なし
```

**🔴最重要発見**：`ask_hub_queue.json`の実物で#960e67（「法人として扱ってよいか」）を
見ると `answer.by="ピタゴラス（機械判断で解決）"`。**有璽氏は一度もこの問いに回答して
いない。** 作業ログの「コーディネーターの差し戻しを受けて」という説明は実物の記録では
裏付けられなかった。AI自身が有璽氏の判断を代行して畳んだ＝WORKING.md冒頭の有璽氏の
指示（選択式にしてほしい・2回目）に反する。B-0473自体の取り消しは不要（データは正しい
ため）と判定。詳細 `~/.vivid-relay/tsuru_row41.log`。Slackへ`notify.tell()`で報告済み
（判断語0件を確認して送信）。

**★同じ対象に手をつけないでください**: なし（検査完了・読むだけだったため対象は無傷）
...
**有璽氏のボタン回答4件は自分で実測した**（`ask_hub.answer_of()`・4件とも `by=yuji38132000`）。
**00_企業マスタ・01_顧客詳細・受付シートへは1文字も書いていない。Notion・kintoneも同様。
Slackへの実投稿は ④ の1件だけ。**

```
④ 受付シート41行目  ✅ 全項目＋照合結果＋隣の行40の処理済みまで添えてボタンで出した → #8330f0
                    ★候補0件を2経路で確認（毎朝の照合／台帳3本を直接検索）。回答待ち
① 毎朝の督促        ✅ 3件を「対応が要る」から外した。★消していない・期限なし
                    実測 前=★対応が要る3件 → 後=対応が要るものはありません＋⏸保留中3件
                    findings_escalate ドライラン0件＝★明朝08:15のSlack督促は出ない
                    戻し方は1手 findings_tracker.py --unpark "<key>"（--list-parked でキーが読める）
② 01から3行削除     ★実行していない。差分・全42列の中身・試算まで
                    ★試算 21組→**9組**（引き継ぎ書の14組は誤り＝B-0008の7組しか引いていない）
                    ★B-0076 と B-0132 は消すと台帳から実体が完全に消える（00は8/31に削除済）
③ 顧客種別          ★実行していない。表A 変更候補14件／表B 判断つかず40件／表C 番号確定5件
                    ★corp_number_fill の「確定16件」は別の母数（59件プールに入るのは5件）
```

**結果の全文** `~/.vivid-relay/ledger4_result.md`
／控え `_backups/*.bak_20260905-park`（5本）・Drive複製 `1iN9yTfByTrIZ5k1hJwljLu2eNj3FRq8Dcm8yT1NOJqw`
（2,068,290バイト・複製側で3行の中身が本物と一致することまで読み返した）

**🔴ビビへ2点**
① **MacBook 側の `~/.vivid-relay/findings_tracker.py` は未配布**（git管理外）。
   `bin/hooks/` 側は commit 済み（edf46b3）なので届く。MacBookで点検系を叩くと旧版が動く
② **②③はつるの検査を通してから。** ★特に②は「消してよいか」ではなく
   「消えるものを見せたうえで、それでも消すか」の確認が要る

**★同じ対象に手をつけないでください**: `~/.vivid-relay/{ledger_report.py, findings_tracker.py,
dashboard_data.py, dashboard_build.py, open_findings.json}` ／ `bin/hooks/findings_tracker.py`
...
検査した版＝新ファイル Drive `version=19` ／ `2026-09-05T08:06:23Z`（rev42）。検査の前後で不変。
**★作った側の報告が失われていたので、rev35 を .xlsx で書き出し現物どうしを1セルずつ突合した。**

```
11社減(3,506→3,495)  ★11組の統合で過不足0。★誤って別法人を束ねた組は 0（11組とも電話が完全一致）
ＨＡＬＥ               ★束ねていない（2行とも値も不変）＝指示どおり
2回かける行            ★12行 → 1行（ＨＡＬＥのみ）。法人番号での重複0件
2周目の条件①（行番号）  ★解けた。1,154行中1,136行が法人番号／名前＋電話で1件に引ける
国税庁に無い番号        ★0件（法人単位3,280・事業所単位3,280を全数突合）
FAXの先頭0落ち         ★直った（2経路）。電話は無傷。事業所単位の変更は188セルのみ・その他0セル
既存3本                無傷（version＋revisions）。控えも中身まで確認した
```

**🔴 有璽氏の判断が要る1点＝ＨＡＬＥ株式会社。** 複数サービス1029行の法人番号 8120001210297 は
国税庁では「株式会社ＨＡＬＥ・エステート（大阪市北区）」＝**別法人の番号**。同じ電話・同じ住所の
2行なので、同一法人で片方の番号が誤っていると読める。**台帳・kintoneと突合する前に決着させること。**

**🟡 渡したあとで直す2件**（架電は止めない）：①新タブの数字3つが実物と不一致（「8組を束ねた」は
実際は11組・M001〜M008 に3組が漏れている）②要判断タブ113行でI列「判断不要」とJ列「○を付けて」が矛盾。

報告全文 `~/.vivid-relay/saleslist_tsuru3.log` ／ 記録 `memory/project_telapo_list_other_services.md`。
**★つるは新ファイルを直していない。修正はピタゴラス側。**

### 【ピタゴラス / mini 2026-09-06】テレアポリスト ①ＨＡＬＥをNG ②既存顧客との重なり ── ✅完了。つる検査待ち

**書いたのは新ファイルだけ。`00_企業マスタ`・見本・抽出元2本・Notion・kintone へは1文字も書いていない。
Slack へも投稿していない。書き込みは RAW。** 16タブ → **18タブ**。

```
① NG   新タブ「★NG_架電しない先」（注記4＋見出し1＋データ5行）。理由は「有璽氏の指示（2026-09-05）」だけ
        元の5行は★消さず「最新ステータス」へ `NG（架電しない）`
        実測：5セルとも入った／全11タブでの NG の総数 = 5
        ＨＡＬＥ株式会社 7120001249725 … 事業所単位_放デイ・法人単位_放デイ
        HALE株式会社   8120001210297 … 事業所単位_放デイ・事業所単位_児発・法人単位_複数
        ★要判断のM行（1155）＋★G030/H065 の4行にも決着文（旧文は消さず「（旧：…）」で残した）
② 候補 新タブ「★NG候補_既存顧客との重なり」★21法人・延べ76行。★機械はNGへ入れていない
        母数＝台帳「会社名がある行」★467社（依頼文の416社は 8/20 時点の数）× リスト11,141行
        キー3本（法人番号／会社名の正規化／電話）。内訳 会社名のみ9・会社名+番号4・3キー4・番号+電話2・会社名+電話2
```

**🔴人の判断が要る3点**（全文 `~/.vivid-relay/saleslist_ng.log`）
① **自社がリストに載っている ── 株式会社ILIFE 6120101064356「ＬＩＦＥ ＳＴＡＮＤ ＵＰ」羽曳野市**
  （事業所単位_放デイ2067行／法人単位_放デイ167行・台帳では B-0034）。**NGへ入れていない**
② **依頼文の前提が違った。**「既存顧客をNGへ」は `project_gamebull_form_sales` に実在するが、
  **同じファイルの後段（2026-08-23 有璽氏）で取り下げ済み**（「既存顧客にも送ってよい」）。
  候補出しは可逆なので進めた。**21法人を実際にNGにするかは判断が要る**
③ **★依頼を1点だけ超えた**：ＨＡＬＥの要判断行は M 以外に G030・H065 の4行もあり、
  「同一なら○を付けて」のまま残ると決着済みを判断しに行かせる（つる3周目🟡②と同型）。
  4行の判断欄にも決着文を入れた。**消していない。戻すのは1手**

**★触っていないことの実測**：控えと全16タブをセル突合 → **差分11セル・その他0セル**。
事業所単位5タブ・法人単位6タブの行数は不変。既存3本は2経路（version+modifiedTime／revisions）とも不変。
営業ワークブックは version 1016・2026-09-04T22:48 のまま＝**書いていない**。
控え `福祉施設アプローチリスト_バックアップ_NG対応前_20260906`（id `1XcVhpWLvFPs…`・1,342,729バイト・
16タブとも実データ行数が本体と一致することを開いて確認）。

**★同じ対象に手をつけないでください**: 新ファイル `1PXBlBraJrbDH0habkDeRGJT4U5NSRE03BsUXrT_ZbDA`

### 【ピタゴラス / mini 2026-09-06】つる3周目の🟡2件＋検索文字列のゆれ ── ✅完了

**書いたのは新ファイルの「★このリストの作り方と限界」3セルと「★要判断_同一法人の候補」124セルだけ。**
書く前にDriveへ複製（`…つる3周目対応前_20260906` id `1fuWUs-…`・1,341,263バイト）し、
**控えを開いて16タブの行数が本体と全一致することまで確認**してから着手した。書き込みは **RAW**。
台帳・Notion・kintone へは1文字も書いていない。Slack へも投稿していない。

```
① 数字3つ  「8組」→★11組（M009 なかの屋／M010 有限会社ライフ／M011 Ｇｏｔｏｓｃｈｏｏｌ を新設）
            ★3,506→3,495 の11行減と M001〜M011 が1対1でつながる形にした（2経路で数え直し）
            「法人名だけ一致 70組82行」→★52組63行（★つると独立に一致。数え方もシートへ書いた）
            「名称の食い違い 45件」→★数え方を併記。★数え方で20倍ちがう（898/69/つる105）
② I/J矛盾  113行 → ★0行。J列は「★解消済み（旧の理由：〜）」で旧文を消さずに残した
③ 検索文字列 ★11行を直した。読み返して引けない行 ★0行（つるの申告は4行・理由は下記）
```

**★つるより多く出た理由＝照合を正規化したかどうか。**つるの4行は機械が正規化して照合した結果、
こちらの11行は**人が Ctrl+F へ貼り付けてそのまま当たるか**で数えた。判定は厳しい方を採った。
**うち4行は「束ねられた」自体が誤り**で、法人番号 9120105008793／4120901045565 は
**全11タブに0件**（控えにも0件・2経路）＝母集団から落ちた行だった。推測で別の行へ結んでいない。

**★触っていないことの実測**：控えと全16タブを読み出してハッシュ突合 → 差分は上記2タブのみ。
事業所単位5タブ・法人単位6タブ・活動FB・テレアポトーク・★修正ログは全一致。
**ＨＡＬＥの行（1155行）は1セルも動かしていない**（読み返して不変を確認）。
既存3本も無傷（見本 v158 ／ 抽出元2本 v18・2026-07-28 のまま）。

**報告全文** `~/.vivid-relay/saleslist_fix3.log`。記録は
`memory/project_telapo_list_other_services.md` ／ `reference_row_reference_rots_on_rebuild.md`。
**★自己採点にしていない。検査（つる4周目）はビビへ返す。**
...
**新設したのは `~/.vivid-relay/stall_watch.py`（705行）だけ。既存ファイルは1本も編集していない。**
**台帳・Notion・kintone へは1文字も書いていない。Slackへの実投稿0件**
（`ask_hub.api_post` と `notify._post` の両方をモック＋`VIVID_NOTIFY_OFF=1` の二重の防止）。
**cron/daily_jobs 未登録**（手順は結果ファイルの§5）。**走行中のつるは殺していない**（実測で生存確認）。

```
見るもの  走行中の claude -p ＋ その transcript（★親＋subagentsの新しい方）の更新時刻
判定      ★生死でなく「無言の長さ」。20分で ask_hub のボタン（1 いま止めてよい／2 まだ待つ）
閾値      ★実測してから決めた。直近3日・transcript51本・14,397ギャップで >12分は0件・最大10.0分
          → ALERT_MIN=20（実測最大の2倍）。判断待ちは6h（ask_hub回答10件の実測：日中最長1.12h）
実測      テスト13件 全合格（単体3／通知の中身／重複抑止／killガード4種／落下時の心拍／ボタン往復4種）
```

**★結果・未検証・cron手順・判断が要る点 → `~/.vivid-relay/stall_watch_result.md`**

**⛔2026-09-05 18:05 ①は解決・穴を1つ潰した（同じセッションで追記）。** 705行→931行・
sha256 `0cae8311…`（控え `_backups/stall_watch.py.bak_layer1_fix_20260905`＝`8d3dbdcc…`）。
**触ったのは `stall_watch.py` 1本だけ。Slack実投稿0件・state未作成・cron未登録・走行中は殺していない。**

```
①解除を外した     層3が完了しているので unblock_pending を削除（2経路で0件を確認）。見張りは残す
②★対応づけを直した  起動時刻の近さ→★指示文の一致へ。psは指示文を切り詰めず全部出す（実測3,069文字）
                   ★同じ秒に2本transcriptが生まれる（17:24:51 層2/層3・17:43:40 層1/営業台帳）。
                   etimeは秒どまりなので±1秒振ると★5点中2点で相手を掴む＝約4割 取り違えていた
③veto を新設       ログ・成果物が新しければ鳴らさない。★非対称に使う ──
                   新しければ生きている証拠になるが、★古くても死んでいる証拠にはならない
                   （ログはまとめ書き＝ビビの発見。実測：層2はログ15分無更新・transcriptは最大1.3分）
④閾値を測り直し    母集団を判定と同じ形（親+subagents マージ）へ。42本・12,951ギャップ・>12分0件
                   → ALERT_MIN=20 据え置き。★母集団ごと引き継ぐこと（数字だけ渡さない）
```

**🔴残る誤検知の場面（正直に）**：30分級の単一ツール実行（大きなrsync/ビルド）は3経路とも古くなる。
母集団に実例が無く、**本物の1件を見るまで塞げない**。詳細と実測値は `stall_watch_result.md` の追記。

**🔴ビビへ2点（①は上のとおり解決済み）**
① ~~**③（判断待ちの解除）が「層3」と重なるかもしれない。**~~ → 解除は外した。
② **②の30分打ち切りは、痕跡がログに残らない**（文言はSlackへ返る戻り値の中だけ）。
  事後に数えられないので「25分で予告」する形にした。`slack_inbox.py` は触っていない。

**★同じ対象に手をつけないでください**: `~/.vivid-relay/stall_watch*`

### 【ピタゴラス / mini 2026-09-05 深夜】つる2周目の指摘①〜⑤を直す ── 着手（書く）

**書いてよいのは新ファイル `1PXBlBraJrbDH0habkDeRGJT4U5NSRE03BsUXrT_ZbDA` だけ。**
見本・抽出元2本は読むだけ。台帳・Notion・kintone へは書かない。Slack へ投稿しない。
書く前にDriveへ複製する。①要判断/修正ログの行番号を法人番号ベースへ ②12組の重複のうち11組を決着
③国税庁に無い法人番号3件 ④FAXの先頭0落ち ⑤検算が何を保証しないかをシートへ明記。

**★同じ対象に手をつけないでください**: 新ファイル `1PXBlBraJrbDH0habkDeRGJT4U5NSRE03BsUXrT_ZbDA`

### 【ピタゴラス / mini 2026-09-05 夜】法人単位6タブを事業所単位から組み直す ── ✅完了。つる2周目の検査待ち

**有璽氏の決定が変わった：法人単位＝「大阪府に事業所を1つ以上持つ法人」**（旧＝法人住所が大阪）。
**書いたのは新ファイル `1PXBlBraJrbDH0habkDeRGJT4U5NSRE03BsUXrT_ZbDA` の法人単位6タブ＋★要判断タブへの追記だけ。**
書く前にDriveへ複製（1,251,457バイト・0でないことを確認）。台帳・Notion・kintone・Slackへは書いていない。

```
                    旧      新              実測の検算
放デイ              215 →   198    事業所数の合計 7,646 ＝ 事業所単位5タブの全行 7,646（過不足0）
児発                 81 →    80    法人番号 13桁でないもの 0件・電話の先頭0落ち 0件
★Ａ型               31 → ★233    ← 379法人を31行に潰したバケツが解消
Ｂ型              1,471 → 1,394    種別タブ⇄複数サービスの重複 0件（法人番号で2経路確認）
移行                 81 →    84    ★府外本社 177社・大阪の事業所 496行が新たに載った
複数サービス      1,392 → 1,517    （つるの「492行」と行では完全一致。社数は束ね方で振れる）
合計              3,271 → 3,506
```

**★触っていないことの実測**：事業所単位5タブは 2422/2205/470/2211/338 で作業前と全一致。
活動FB（2行・IFERROR の数式のまま）／テレアポトーク44行／★修正ログ1,073行 も不変。
既存3本は2経路（Drive version+modifiedTime ／ タブ構成）とも不変＝無傷。

**🔴人の判断が要る2点**
① **旧にあって新に無い法人 92件。**うち87件は「本社は大阪だが大阪府内に事業所が無い」
   ＝新しい定義の当然の帰結。**残り5件は法人名は一致するが番号が違う**＝要判断タブへ出した。
② **★要判断_同一法人の候補 が 287行 → 1,145行**（909グループ）。既存287行は1行も消していないが、
   **その「どのタブの何行目」は組み直しで無効**（区切り行に明記）。旧130グループのうち47は解消済み。

**★事業所数の列は「大阪府内の事業所数」**にした（元データは全国の数）。列名は見本のまま、
**見出しセルのメモ（note）**で意味を明示。★見出し直下に注記行を足す案は、フィルタと
「行数−見出し」の数え方を壊すので採らなかった。**行が要るなら1手で足せる。**

**★同じ対象に手をつけないでください**: 新ファイル `1PXBlBraJrbDH0habkDeRGJT4U5NSRE03BsUXrT_ZbDA`

### 【ピタゴラス / mini 2026-09-06】承認ダイアログをSlackから返せるようにする（有璽氏「確実に早くして」）── 着手（書く）

**★直下の 9/5 のブロックは実物に1バイトも残っていない**（`bin/hooks/hook_permission_slack.py` と
`~/.vivid-relay/` 側の sha256 が一致 `5545168a…`／git 側 mtime が 8/20 01:50）。**引き継ぐ。**

```
★書いてよい  bin/hooks/hook_permission_slack.py（★正本はこちら）／その控えを _backups/ へ
              ~/.vivid-relay/hook_permission_slack.py（配布先）
★書かない    🔴 ~/.vivid-relay/ask_hub.py（無改修）／slack_socket.py
              ~/.claude/settings.json（登録は人の手・要承認。足す全文だけ書き残す）
              台帳（00/01/02/40）・受付シート・Notion・kintone
Slack         ★実投稿しない（ask_hub.api_post と notify._post の両方をモック）
出口          ~/.vivid-relay/approval_dialog_result.md
```

**🔴 二重に走っている（2026-09-06 09:0x 発見）。** 出口に指定された
`~/.vivid-relay/approval_dialog_result.md` が **09:00 に既に作られていた**（125行・途中経過）。
**別セッションのピタゴラスが同じ依頼を並行で受けている。**着手宣言はどこにも無かった。
→ [[reference_two_sessions_built_the_same_thing]]

```
★こちらの対応   上書きしない。追記だけにする。実測が返ったら「実測（ピタゴラス2）」節を足す
★持ち物が違う   相手＝CLAUDE_CODE_ENTRYPOINT での対話/非対話の分岐・設計案
                 こちら＝timeout の実装コード／★時間切れは answer:{}＝「意見なし」で
                 勝手に許可されない／setup_hooks.sh は timeout を巻き戻さない／
                 DMの実物8通（8通中5通が `Bash ls` の騒音）
★合わせると強い  ビビに1本へ束ねてもらうこと
```

**★同じ対象に手をつけないでください**: `bin/hooks/hook_permission_slack.py`（両機）／
`~/.vivid-relay/approval_dialog_result.md`（★追記のみ・上書き禁止）

### 【ピタゴラス / mini 2026-09-05】承認ダイアログの通知を直す（ビビ依頼・有璽氏の指示）── ⛔落ちている。上のブロックが引き継いだ
...
### 【ピタゴラス / mini 2026-09-05 夕】つるの検査①〜⑤を新ファイルへ反映 ── ✅完了。つる2周目の検査待ち

**書いたのは新ファイル `1PXBlBraJrbDH0habkDeRGJT4U5NSRE03BsUXrT_ZbDA` だけ。**
見本・抽出元2本は読むだけ（2経路で無傷を再確認：Drive の modifiedTime+version ／ タブ構成）。
台帳・Notion・kintone・Slackへは1文字も書いていない。**書く前にDriveへ複製（1,214,489バイト・
中身が入っていることを確認してから着手）。** 13タブ→**15タブ**（★要判断・★修正ログを新設）。

```
① 活動FB   ✅ 見本と同じ数式の集計行＋見出し行。放デイの実績値(24/16/5/1)は消えた
            ★1点だけ見本と変えた：率3セルを IFERROR で包んだ（B1=0 で #DIV/0! が出るため）。戻すなら1手
② 法人番号  ✅ 壊れた784行を国税庁全件(5,827,771行)で照合 → 717行を復元・67行は空にした
            2経路一致587 ／ 1経路のみ130。読み返し実測：型①残0・桁数≠13残0・検査数字NG残0
③ 名寄せ    ✅ キーを直した（12桁は zfill(12)＋検査数字。★zfill(13)は誤り）
            法人番号が一致する重なり 8組→★20組。★行は1行も動かしていない（理由は下の🔴）
            候補は「★要判断_同一法人の候補」タブへ130グループ287行（旧81件は網羅していなかった）
④ 電話      ✅ 数値化で先頭0が落ちた288件を復元。読み返しで数値の残り0件。★ハイフンは付けていない
⑤ 固定列    ✅ 事業所単位=8・法人単位=2・basicFilter を11タブへ。見本と同じ
⑩ 見本09:40 ✅ リビジョン251⇔254を.xlsxで突合 → ★7タブとも差分0行＝セルの値は変わっていない
```

**🔴 ①〜⑤より重い問題を1件見つけた（当方では直していない・設計の分岐）**
**法人単位_就労継続支援Ａ型 の31行は、約379法人を潰したバケツ。**抽出元の側でＡ型の名寄せが
壊れた法人番号のまま行われている（実測：抽出元の大阪Ａ型37行の事業所数合計=468 ⇔
事業所単位Ａ型470行のユニーク法人名379／6行が事業所数37･60･76･40･44･40を持つ）。
**取り戻すには法人単位を事業所単位から組み直すことになり、有璽氏の決定「法人単位＝法人住所」を
変える。だから触っていない。**事業所単位Ａ型（470行）は潰れていないので架電はできる。

**⛔2026-09-05 夜 解決。** 有璽氏が母集団の決定を変え（法人単位＝大阪に事業所を持つ法人）、
事業所単位から組み直した。**Ａ型は 31行 → 233行。**上のブロック参照。

**★同じ対象に手をつけないでください**: 新ファイル `1PXBlBraJrbDH0habkDeRGJT4U5NSRE03BsUXrT_ZbDA`
**★恒久スクリプトは保存していない**（役割の検問がこのセッションを「ビビ」と誤検出し .py を書けない）。
手順は `memory/reference_recover_exponential_corp_number.md` の「復元は3経路で組む」節に落とした。

### 【ピタゴラス / mini 2026-09-05】テレアポリストを他サービス種別へ広げる ── ✅完成（13タブ・10,982行）
...
**既存3本（見本・抽出元2本）は読むだけ。台帳・Notion・kintone・Slackへは1文字も書いていない。**

```
事業所単位（事業所住所=大阪）              法人単位（法人住所=大阪・名寄せ後3,271法人）
  放デイ2,422 児発2,205 Ａ型470              放デイ215 児発81 Ａ型31 Ｂ型1,471 移行81
  Ｂ型2,211 移行338 ＝ 合計7,646             ★複数サービス1,392  ＝ 合計3,271
補助  活動FB 0行（見出しだけの器） ／ テレアポトーク65行（見本の複製＋注記1行）
```

**実測**：読み返して13/13タブとも行数一致・住所の県外混入0件／
既存3本は**2経路（Drive の modifiedTime+version ／ Sheets のタブ構成）とも一致＝無傷**。
共有は不要だった（**認証は有璽氏本人のOAuth**・permissions.list で role=organizer を確認）。

**🔴人の判断が要る1点**：名寄せで**束ねなかった候補81件**（法人名は一致するがキーが違う）。
誤って束ねる方が有害なので別法人として扱った。全件は `~/.vivid-relay/saleslist_pitagorasu.log` の末尾。
**残＝⑤テレアポトークの種別ごとの書き分け（保留のまま）。**

**★踏んだもの（次に触る人へ）**
- **法人番号が同じ列で文字列と数値の混在。**先頭の0が落ちており、**13桁ゼロ埋めで正規化しないと
  同じ法人が別物に見える**（正規化前1,288→後1,292）。
- **共有ドライブのファイルは Drive API が `supportsAllDrives=True` 無しで404を返す。**
  Sheets API では読めるので「共有されていない」と誤診しやすい。
- **read() は行末の空セルを詰めるので行長が不揃い。**書く前にパディングが要る（1回落ちた）。

**★同じ対象に手をつけないでください**: 新ファイル `1PXBlBraJrbDH0habkDeRGJT4U5NSRE03BsUXrT_ZbDA`

---

**（判定フェーズ・完了済みの記録）** ── ビビ依頼「作れそうか確認して」。
**3ファイルとも読むだけ。台帳・Notion・kintone・Slackへは1文字も出していない。**

**結論＝作れる。**列の見出しは見本と抽出元で完全一致・入力規則/条件付き書式/保護は3タブとも0件。
**ただし見本の「大阪絞り914」と同じ基準では作れない**（下記）。

```
大阪絞り914の正体   法人単位の行7853〜9359（法人住所=大阪府の連続1507行）から順序を保って抜いたもの
                    法人住所=大阪府 1507 → 指定機関名∈{大阪市,大阪府} 1020 → ★9市を除外 914
                    ★9市（枚方/豊中/八尾/和泉/箕面/富田林/門真/豊能町/能勢町）の除外に説明がつかない
                    ★「仮メモ欄」に「大阪法人 900」とあり、件数ありきの手選別の可能性（推測）
```
...
**⛔この判断待ちは解消した。** `#0de8d1` は 2026-09-05 11:34 に有璽氏が
「**同じ見た目にする**」を押している。実装は下のブロック。

**★同じ対象に手をつけないでください**: `~/lifestandup-wp/theme/lifestandup/` ／
`~/design_handoff_v2.0/`（読むだけ）

### 【リリス / mini 2026-09-05】活動ブログを記事デザインへ揃え・既存ブログ5本を移送・仮公開の出し直し ── ✅実装とpush済み（9921865）。デプロイ確認中

有璽氏の指示3件（①デモのお知らせはダミーのままでよい ②既存ブログを4〜5件 実データで移す
③仮公開サイトを出し直す）。**本番サーバー・WP管理画面・DNS／台帳・Notion・kintone へは
1文字も書いていない。** 既存サイトへは**公開のREST APIで読みに行っただけ**（認証なし・書き込みなし）。

```
触ったもの  theme/lifestandup/{single.php, single-news.php, functions.php,
                              assets/css/news-detail.css}
            crawl_static.py ／ README.md（15節を新設）
            check_overflow_blog.py・check_overlap_blog.py（新規・検証用）
            _tools/blog_import.php（新規・★git管理外。冪等・--clean で戻る）
```

**🔴 いちばん大事な発見 ── 写真の並べ方が2種類あり、多数派は Jetpack**

```
既存サイト134本を全部数えた（実測）
  タイル状ギャラリー(Jetpack)を含む記事   ★78本 / 134本   ← 多数派
  標準のギャラリーを含む記事                28本 / 134本
★別のマークアップなので、受け皿が無いと写真が縦1列に潰れる（潰れているのを見てから直した）
★列を指定していないギャラリーが214/227個。WordPressの決まりは「3枚以上なら3列」＝
  枚数で変わる。2列固定にすると、既存サイトで3枚並んでいた回が2列になる
★丸・正方形の意匠は残した（書き手が選んだものなので潰さない）
```

**⛔14節の元になった調査への訂正** ── 既存3本しか開かずに「動画・表・埋め込みは0件」
としていたが、**134本全体では実在する**（動画5本・ファイル5本・埋め込み1本・ボタン1本）。
今回移した5本には入っていないので、**この5本では確かめられていない。**

**実測**：ギャラリーの列が枚数`[2,2,2,1,3,1,3,3,6,9,1]`と完全一致／記事の高さ
27,385px→5,979px／横あふれ15通り0px／重なり9通り0件（道具が生きていることも別ページで確認）／
**PC幅1440は固定ページ21本＋お知らせの記事1本の出力 sha256 が22本とも一致**。

**★踏んだもの（次に触る人へ）**
- **編集した場所とWordPressが読む場所は別。測る前に必ず `python3 deploy.py`。**
  これを飛ばして測り「変わっていない」を1回信じかけた。
- `crawl_static.py` の `fetch()` が**日本語ファイル名で落ちていた**（一覧のサムネイル2枚＝実害）。
- **★検証サーバを1回止め忘れた**（別セッションに指摘された）。
- **★役割の検問がこのセッションを「ビビ」と誤検出して .py を書けない**
  → [[reference_hooks_enforce_what_discipline_cannot]]。回避せず、道具はPHPで書き、
  .py はサブエージェントへ渡した。

**★本番の移行で要る設定（有璽氏の手）── 2つセット**：「設定 > 表示設定」で
①ホームページ＝トップ用の固定ページ ②投稿ページ＝`/blog/`。
**①を飛ばすとトップが front-page.php でなく投稿一覧になる／②が無いとヘッダーの
「活動ブログ」がトップへ飛ぶ**（どちらも実測で踏んだ）。

### 【リリス / mini 2026-09-05】ビビ依頼4点（①ブログ中身404 ②5本のまま ③IGリンク統一 ④IG API調査）── 着手
...
```
いまの状態  https://lifestandup-preview.vercel.app は★生きている（401）。
            ただし中身は★前の版（ブログは入っていない）。落ちてはいない
残り        新しい版のデプロイが Vercel 側で "Building…" のまま。alias を張れずにいる
```

**★分かったこと（次に触る人へ・memory にも書いた）**
- **GitHubへ push すると自動デプロイが走り、固定URLを空のデプロイへ奪う。**
  9/5 の404の説明がつく。リポジトリのルートへ `vercel.json` を置いて止めた。
- **`vercel ls` の `UNKNOWN` は「ビルド中」の表示。詰まりではない。**
  ★一度「順番待ちが詰まっている」と誤診し、消す必要のないデプロイを3つ消した。
  **極小のもの（index.html 1枚）を同じ場所へ通したら数秒で READY** ＝ 場所は正常。
- 写真は原寸で置かない。`sips --resampleWidth 1024` で 48MB→31MB。
  **既存サイトが配信しているのと同じ大きさになり、見え方も本番に近づく。**

**★同じ対象に手をつけないでください**: `~/lifestandup-wp/`（theme・static-preview・_tools）
／Vercelプロジェクト `lifestandup-preview`
...
### 【チョッパー / mini 2026-09-13】★Cの適用に台帳を作る（有璽氏「チョッパーでいいや。それで進めて」）── 走行中

**★14体目の担当・1本目の仕事。** 有璽氏の指摘「保存したものは反映されるんだっけ？
ログとしては残らないみたいなこと言ってなかった？」が正しかったことへの対応。

```
穴1  editor_apply.py:512 は必要な情報をprintしているのに ★crontabが `> /dev/null` で捨てている
穴2  editor_apply.py は ★git commit を1度もしない（grep 0件）＝未コミットのまま積み上がる
直す ①crontabの1行 `> /dev/null` → `>> editor_apply.log`
     ②適用したら名指しでadd→commit（★git add -A禁止・pushはしない・空コミットも作らない）
```

**★同じ対象に手をつけないでください**: `~/.vivid-relay/editor_apply.py` ／ mini の crontab
**★チョッパーには触らせていない**: ポート8750 ／ `_tools/wordpress/wp-content/lsu-reports/`
（★sweep_widths.py PID 80324 が使用中）


### 【ビビ / MacBook 2026-09-13】幅を掃いて崩れを見つける手順を定める（有璽氏「1と2をセットで進める」）── ★リリス(mini・PID 68608)が①②を実測中

有璽氏の決定（2026-09-13）：**①幅を連続で掃く ②結果を★Cの画面へ出す をセットで進める。
★手順が固まってから担当もしくはスキルにする（順序は同意を得た）。★まず手順を定める。
対象は LIFE STAND UP のホームページ。**

**★目的（有璽氏の言葉）**：「PCサイズから**サイズを狭めていく**。狭めていって最終的に
スマホバージョンになった時も**表示が崩れないようにするための設定**。それを**視覚的に
こちらが確認しながら**進める」＝有璽氏はこれを**レスポンシブ／オートレイアウト**と呼ぶ。

**★実測で分かった土台（転用できる）**
```
check_overlap_all.py:28  WIDTHS = [375,390,414,768,820,1366,1920]  ← ★定数。差し替えれば連続掃き
                  :20-21 ページは build_pages.py の URL_MAP ＋ EXTRA_CASES 4本
                  :49    ローカルWP(8750)を E.serve で立てて測る／lsu-collect.php で集計
崩れを数える部品   ★27本（横あふれ13・重なり9・括弧・リンク・PC不変ほか）
★C               幅切替7点・保存形式A(%/calc 全幅)／B(この幅だけ)・lsu-inspect.html
```

**✅有璽氏の決定（2026-09-13）**：A=②横あふれ＋重なり ／ B=②代表5ページ ／ C=①20px→2px。
**★リリスを mini で起動済み**（PID 68608・ログ `~/.vivid-relay/lilith_20260913.log`・
指示文 `~/.vivid-relay/lilith_sweep.txt`・出口 `~/.vivid-relay/sweep_result.md`）。
作るのは新規 `~/lifestandup-wp/sweep_widths.py` のみ。**★既存27本は書き換えない・
theme配下のCSS/PHPは1文字も変えない（今回は測るだけ）。**
代表5ページ＝`/`・`/stand-up/`（★有璽氏が見ていた）・`/recruit/`・`/stand-up/programs/`・`/news/`。

**★この段階では何も実装していない。**読むだけ。本番サーバー・本番WP管理画面・DNS／
台帳・Notion・kintoneへは1文字も書いていない。

**★同じ対象に手をつけないでください**: `~/lifestandup-wp/check_overlap_all.py` ／
`lsu-editor.html` ／ `memory/project_web_build_rules_asset.md`


### 【ビビ / MacBook 2026-09-08】かわちばなし（地域イベントポータル）の画像18枠を作り直す ── ✅完了
...
**✅18枠すべて差し替え済み**（実測：cdn.gamma.app 0件／uploads参照12種＋instaSlots 6枚／
Artifact側は相対パスのsrc 0件・埋め込み21件）。他社へ渡せる形も用意した：
**https://drive.google.com/file/d/1wqKAcLIHq-2Zns1vqtH89pJ_kcPTY907/view**
（★Artifact は Claude を使わない相手に届かない → [[reference_artifact_is_not_public]]）

**🔴同じフォルダに置いた控えが消えた**（19時ごろ Claude Design 側がフォルダを整理したとみられる。
...
### 【ビビ / MacBook 2026-09-09】ロゴを主役にする改修（案D）── ★Claude Design 側が作業中

有璽氏「せっかくロゴマーク素敵なのに活かせてない」。**★作るのは Claude Design 側。
こちらは診断と指示文と検査**（9/8にローカルの.dc.htmlを書いて全部無効になった反省）。

```
渡したもの  ①ロゴを主役にする指示（A=hero／B=虹の波線・6アイコン・3つの丸・4市ピル）
            ②★水彩アイコン6枚（~/kb_icons/配置用/・こちらのOpenAIで作成・$0.035）
             ＝向こうはGammaのクレジット0で作れないため。財布が別なだけだった
仮置き      「伝えたい思い」3つは★有璽氏が仮承認しただけ。確定として引用しない
触らせない  写真18枠・文言・4市の色・書体・構成順
```

**★戻ってきたときの検査は、ローカルのファイルではできない**（クラウドが正本）。
有璽氏に①画面のスクショ ②Claude Design からの書き出し のどちらかをもらう。

**★同じ対象に手をつけないでください**: `~/Downloads/地域イベントポータルサイト設計/` ／
`~/kb_images/` ／ `~/kb_icons/`のみ
...
- **【ビビ / MacBook 2026-09-14 24時】LIFE STAND UP ── ★明日の再開点（有璽氏は帰宅・離席中に進めた分）**
  ...
  ✅ イベント区分（案A）    ★本番へ category「イベント」(slug=event・id=292) を新設
                          ＝これから開く催しの告知。★書くのは本部事務・現場の運用は変えない
  ✅ 保護者の声2本         お知らせ一覧から外した（★記事は消していない・専用ページへ寄せた）
  ✅ 手順書               ~/Downloads/LIFE_STAND_UP_本番テーマ_20260914/イベントのお知らせを出す手順.pdf
  ✅ zip 0.2.2            同フォルダ ★有璽氏へ送付済み。★明日アップロードしてもらう
  ```
  **★明日いちばんに触るもの**
  ```
  1  zip 0.2.2 をアップロード（★公開はされない＝有効化していないため）
  2  ★写真のチェック（有璽氏）── これが終われば公開まで行ける
  3  手順書を本部事務へ渡す
  4  ★お役立ち情報の記事づくり（こちらの残作業・公開直前にというご指示）
  ```
  **★本番WPの鍵が入った**（2026-09-14・有璽氏がSlackで送付）。`~/.vivid-relay/config.env` の
  `LSU_WP_USER` / `LSU_WP_APP_PASS`。**★miniにのみ在る。MacBookへは未配布。**
  記事とカテゴリーの読み書きが通る（実測）。★`users/me` だけ403＝サーバ側の保護・実害なし。
  ★テーマの**アップロードも有効化も人の手**（`/wp/v2/themes` は GET のみ・実測）。
  **🔴 鍵をこちらが1度ログへ出した。**有璽氏の判断＝A（このまま使う）。
  → [[reference_tool_results_cache_keeps_secrets]]
  **★控え** `~/.vivid-relay/lsu_cat/backup_20260914-234539/`（記事132本・カテゴリー5件）
  **★実測**：公開中のサイトへの影響0（トップに「イベント」0件・主要3ページとも正常）
  **★同じ対象に手をつけないでください**: 本番WPの category「イベント」(id=292) ／
  `theme/lifestandup/{functions.php, page-templates/news.php}`

- **🔴【全セッション共通 2026-09-14】`~/lifestandup-wp` に★未コミット30件が積み残っている**
  ```
   M .gitignore ／ M redeploy.sh          ← 他セッションの書きかけ
  ?? _chopper_*.py ／ _verify_*.py ／ _work/ など28件  ← 検証用の一時ファイル
  ```
  **★担当は全員終わっている（走行中0件）のに、誰も片付けていない。**
  当方は**触っていない**（他セッションの書きかけを commit / stash しない）。
  ★明日、中身を見て「消す／`.gitignore`へ入れる／commitする」を決めること。
  → [[reference_two_sessions_built_the_same_thing]]（数えるもの5つ目＝未コミットの積み残し）
...
- **【ビビ / MacBook 2026-09-14 23時】オレンジワークス ポートフォリオ `/works/` を本番WPへ実装 ── 着手（読む→下書きで書く）**
  ...
  - **★9/14 23:5x 有璽氏 退勤。明日の再開点**：
    ...
    ②✅「102号室」33か所も有璽氏の回答「全ページ101」で修正済み（#6eeae8 closed・控え2本）
    ⑤✅表記：空白入り「オレンジ ワークス」4件＋privacy を「オレンジワークス藤井寺」「リアンライフ株式会社」へ（公開41ページで実測0）
      ✅#6eab01 略称も全部統一（公開42ページで0）／✅#65db27 recruit 雇用主→リアンライフ株式会社
    ⑥✅一覧は /works/(14229) を Elementor 化し「テーマ投稿リスト」で実装。真因＝REST投入の work に _exclude_in_works_list が無い
      ＝管理画面で1回保存すれば付く。★1件(14103)で保存＋公開→一覧に出るのを実測。いまは非公開へ戻した
      ✅#fc8a23＝一覧と実績42件を公開・ご依頼ページは非公開。★9/15 07:15 実績42件を公開（外から200が42/42）
      ✅9/15 08:17 41件の保存し直し完了 → 一覧 /works/ 公開（未ログインで42件・2経路で実測）。/works/service/ へのリンクは外した
        ★残：ご依頼ページ(14230)は料金確定まで非公開／Desktop側SEOへの連絡
      ★9/15 追加で着手（有璽氏の依頼）：①一覧の見切れ＝テーマ設定 works_list_bricks_proportions_size を 4/3→元の比率へ
        ②実績の追加 ③本体のヘッダー(Main Menu 29)・フッター(Footer Menu 2=32 ほか)へ「芽育クリエイティブラボ」を★追加のみ
        ④お知らせの改修（まず現状調査・読むだけ）。★Desktop側SEOセッションへ：メニュー29/32を触るので同時に触らないでください
        ✅①元の比率へ切替済み（42件・重なり0を実測）✅③ヘッダー/フッターへ追加済み ⏸②材料の置き場を有璽氏に確認中 ⏸④方針を相談中
        ★9/15 決定：⑤書体＝★Zen Kaku Gothic New（M PLUS 2 から訂正・反映済み／Elementorサイトキット11573の12か所）／④お知らせ＝カテゴリー整理と日付表示(テーマ設定 blog_date)から着手
        ／②材料は受け取りフォルダ ~/orangeworks-portfolio/source_追加_20260915/ で受けて加工まで当方
        ✅9/15 サムネを4:3の枠に全体を収める形へ（詰めない版・戻す台帳あり）✅ブログ化：ナビ「ブログ」＋5区分・30本振り分け
        ⏸残：芽育クリエイティブラボ公開のお知らせ記事／追加材料の受け取り待ち／Desktop側SEOへの連絡（住所・名称・新ページ・カテゴリー名）
        ★9/15 10時 着手：①サムネ4:3（詰めない版）を採用確定→追加分も同じ作り方にする道具化 ②/news/ に5区分を選べる欄
        ③お知らせ記事「芽育クリエイティブラボのページを作成しました」を★下書き＋アイキャッチ画像を生成（OpenAI・有璽氏の依頼）
        ✅9/15 15時 自社Web4件とお知らせ記事14458を公開・/works/ は46件（一覧の表示件数42→200）
        ✅9/15 14:3x 4件の画像を作り直し（影・銀色の端末・ページ全体シート・制作年2026）／/works/ 作品名 26→23px
        ✅9/15 芽育ラボの色＝B（OWの緑#1C6835）で確定・見出し画像と記事画像6枚を差し替え済み
        ✅9/15 16時 見本画像を公開29件へ反映・未掲載16件＋OWリーフレットを★下書き・work-310を2件に分割。✅16:15 承認4件＝バナー/上田さん名刺/ステーションロゴを★下書き(14760/14764/14768)・MS名刺表裏をwork-240へ追加(公開ページで実測)。✅16:55 一覧を区分順(Web→グラフィック→ロゴ→企業ビジュアル→名刺→年賀状→イラスト)へ・66件の日付を振り直し(台帳 wp_reorder_20260915-165505.json)。✅17:02 work-110を2件に(670は★下書き)・260/270の見本画像を差し替え。★E 45件のイラストは未確認。✅17:16 下書き21件を公開(一覧67件)。✅18:4x E45件のA+Bを8件の実績として公開(680〜750)・E07をwork-610へ。✅19時 保存し直し完了・一覧75件。⏳110の分割方法と要確認4件は判断待ち／下書きの公開は有璽氏のOK後
    ★有璽氏の恒久指示「離席中も実装できるものは進める」
...
- **【リリス / MacBook 2026-09-12】JFBI 調査研究サイト（1ページ・静的）を実装しVercelへ ── ✅完了。公開済み**
  ...
  - 出力 `~/jfbi-research-site/`（引き継ぎ書 `README.md` 同梱・★.vercelignoreで非公開＝404を実測）
  - **実測**：横あふれ16幅すべて0px／デザイン参照と全テキスト208か所を突合し**残る差は3点のみ**
    （どれもハンドオフREADMEの確定値＝lead16px・ボタン14px・ナビ下線なし）／
    11セクション中9つが高さ完全一致(±0px)／画像15/15・写真4点はWebP／mailto7項目を実物で確認
  - **🔴作業中に自分で作って自分で見つけた不具合2件**（どちらも修正済み・本番で解消を実測）
    ①`img`に`width/height`属性があると`height:auto`が無いだけで`aspect-ratio`が丸ごと無視される
    ②**`hidden`属性は`.stack-24{display:flex}`に負ける。** 非表示にしたはずの「協力者・協力団体」の
    見出しだけが★公開面に出ていた。`[hidden]{display:none!important}`で解消。
    ★`el.hidden===true`を見て「非表示」と断定したのが誤り。**高さ0を測るまで断定しない**
  - **✅9/12 追加対応3件**（有璽氏の指示）
    ①長田教授の写真を**70%へ縮小**（`.who__nagata .fig{width:70%;max-width:381px}`）。
    ★max-widthが要る＝2カラム→1カラムの折返し帯(600px前後)で列が広がり70%でも原寸を超える。
    **実測17幅すべて原寸以下（最大1.00倍）・横あふれ0px**。トリミングは不要と判断（素材比0.747≒3:4）
    ②**独自ドメインを実測で調査**（jfbi.com/jfbi.orgは取得済み・.jpはVercelで買えない）。
    ★最短は`jfbi.vivid-global.com`＝**費用0・DNSに1行**（vivid-global.comは既にVercel登録済み・
    DNSはさくら`ns1.dns.ne.jp`・当該サブドメインは未使用を実測）。README に3案と手順を記載
    ③アクセス解析は**入れない**で確定
  - **✅9/12 追加対応2件**（有璽氏の指示）
    ④**協力団体の掲載欄を表示**（`SHOW_PARTNERS=true`・中身は`{{ }}`のまま）。
    ★隠さず枠を出す＝先方へ「このページのここに載ります」と実物を指して依頼できる。
    **先方へ投げる依頼文＋記入シートをPDFで作成**→`~/Downloads/JFBI_協力団体の掲載依頼_20260912/`
    （メール文面はコピー用.txtも同梱）。★紹介文の字数は**実測で決めた**＝団体が2つ並ぶと
    1件の幅が半分になり40字=2行/70字=3行。依頼は「全角40字程度」
    ⑤**✅独自ドメイン開通** https://jfbi.vivid-global.com/ （有璽氏がさくらでCNAMEを1行追加・
    Vercel側の登録と証明書は自動）。**★人に渡すURLはこちらへ変更**。vercel.appも生きているが案内には使わない。
    ★og:url/og:imageの3か所と**依頼文書3点のURLも新ドメインへ差し替え済み**（実測：旧URL残0件）。
    ★証明書発行の直後はエッジにより200と接続失敗が混ざる＝**1回の失敗で「未開通」と判定しない**
  - **✅9/12 未確定4か所が決着**（有璽氏）。★値をもらったのは1つだけで**残り3つは消す判断**だった
    ①報告会=**2027年2月予定**（埋めた）②同意書=**要相談**→公開面から外しコメントへ退避
    ③承認番号=**無い**→消した ④アクセス解析=**無し**→前後の空白ごと消して文を繋げた。
    ★**「無い」は未確定でなく確定**。`{{ }}`を出し続ける理由にならない→[[feedback_dont_wait_for_the_full_set]]
    ★画面に残る`{{ }}`5件は**全部 協力団体の欄**（script.jsが描画・意図どおり）。
    ★`grep '{{' index.html`では出ない＝**1経路の文字列検索で「残っている/消えた」を断定しない**
  - **✅9/12 追加対応2件**（有璽氏の指示）
    ⑥**表記の統一**「一般に働いている方」→**「一般企業にお勤めの方」**。★名指しは見出し1か所だが
    実物を数えて**同じ人を指す3か所**を揃えた。★「一般就労されている方・一般成人」「一般の就労支援」の
    **2件は研究の分類群の定義なので触っていない**＝揃える対象は文字列でなく**同じものを指す語**
    ⑦**協力団体への連絡文面を作り直した**（`~/Downloads/JFBI_協力団体の掲載依頼_20260912/`）。
    ★宛先も目的もこちらの想定と違っていた＝実際は**仲介者へ「調査研究の名称を定めた」報告と確認依頼**＋
    その方から鶴田さんへ共有＋**鶴田さんのNPO法人の情報の依頼**。名称の理由・意味は
    **`spec/04_名称と法人化の検討メモ.md` の記述に沿って**書いた（推測で組み立てない）。
    旧版は`_参考_掲載団体へ直接送る場合の文面`として残置
  - **★残**：連携NPOの情報は先方の回答待ち／同意書の取得方法は有璽氏が相談中／
    文面の宛名・差出人・「機構」の説明が意図と合うかは有璽氏の確認待ち
  - **✅9/24 有璽氏の修正2件（本番へ反映済み）**
    ①掲載文 ── 詳細ページ「主な活動」。01へ商品パッケージの例「三河武士団子」「むら麦緑茶」を追記／
    02は「岡崎産の「むらさき麦緑茶」のように」→「岡崎産の**むらさき麦を使用した商品**のように」。
    ★原料＝むらさき麦・商品名＝むら麦緑茶の書き分けは有璽氏の指示（先方の原文は両方「むらさき麦緑茶」）
    ②並び ── 協力団体をメンバーと同じ形へ（`.partners-grid`・1行2件・3件目は2行目の左）。
    ★`auto-fit` は1件のとき列を畳んでカードを全幅へ伸ばす＝これが「形が違う」の真因
    → [[reference_grid_collapses_when_only_one]]。**実測：1440pxで member[0] と partner[0] が
    ともに x=192/w=516（ローカル・本番の2経路）／横あふれ 10幅×3件数で0px**
  - **✅9/24 先方から写真18枚＋ロゴ2枚 → ★10枚を詳細ページ「活動の様子」へ掲載（本番反映済み）**
    **★4枚は外した**：①仕入れの型番・数量・**単価135円・値上がりの注記が焼き込まれた1枚**（社内の数字）
    ②他社（丸忠）の店名・商品・価格が主役で a’s の要素が写っていない1枚 ③同一ファイルの重複1枚
    ④チラシ2枚（載せる大きさでは文字が読めない）。**ロゴ2枚は掲載中のものと同一**（バイト数一致）
    **★顔の写り込み0件。原本は `_source/partner-as/写真原本/`（非公開・本番で404を実測）**
    ★つまずき＝縦長パッケージが枠に切られて商品名が欠けた → [[reference_grid_collapses_when_only_one]]
    実測：写真20ファイルとも200／8幅で横あふれ0px・はみ出し0／WebPを選ぶ10/10／切られ10%超0件
    **✅9/24 夕 有璽氏「比率を全部同じものに統一できる？」→ ★枠を1種類（3:4）へ・中身はcontain**
    ★基準に挙がった「のぼり(縦長パッケージ 1:3)」そのままにはしていない＝横長の作品が枠の
    1/3以下になるため。**実測：枠の種類 4→★1（1440px 336x448）／切る指定 8枚→★0枚／
    3幅(1440/768/390)とも横あふれ0・読めない0。**比率が揃ったので並び順も意味の順へ戻した
  - **✅9/24 夜 2団体目＝リライフ訪問看護ステーション（精神科訪問看護・大阪府柏原市）を作った。★本番未反映**
    有璽氏「こちらの事業所も協力団体に追加したい。WEBサイトから掲載文章を抜粋して」。
    ★本文は公式サイト https://relife2022.com/ から抜粋（言い換えていない）。ロゴ2枚は受領。
    **★運営は合同会社フーレ（代表社員 長尾剛）＝ふくち。グループとの関係は先方サイトから読めない。
    いまは「合同会社フーレの事業／大阪府柏原市」とサイトの事実だけ書いている。**
    ✅9/24 有璽氏「問題ないです」→ **本番反映済み**。実測：協力団体2件 x=192/732・w=516
    （メンバーと同じ）／ロゴ・リンク2本とも正／詳細9節・画像0壊れ／原本は404（非公開）／
    横あふれ 1440・768・390 ×2ページ で0。★2団体目が入り、枠は3件目からも横に並ぶ
  - **⏳ リライフの写真 ── ★有璽氏が先方へ送るのを待っている**（9/24 有璽氏「B 先方から出してもらう」）
    ★サイトからの転載はしない：7枚中5枚が顔写真（肖像権）／2枚は素材サイトの疑い（★Exifが空で判定不能）。
    依頼一式 `~/Downloads/JFBI_リライフ_写真の掲載依頼_20260924/`（読む順に0〜3の4本・文面はコピーして貼るだけ）。
    候補7枚の控えは `~/jfbi-research-site/_source/partner-relife/サイト写真_候補/`（★非公開）
  - **触っていない**: 台帳・Notion・kintone・Slack／他案件（lifestandup-wp・かわちばなし）
  - **★同じ対象に手をつけないでください**: なし（作業完了）
- **【ビビ / MacBook 2026-09-12】AI社員オフィス＝★会議室 ── ✅要件確定。第1版(静止)を公開済み**
  - **★有璽氏が方向を確定（会話で回答）**：優先は ①本人が使う ②社内 ③外部（★福祉施設の事務を
    会議室の形で見せる）。指示は **Slackと画面の両方＝入口2・台帳1**。**★スマホ必須**（外出先から
    指示・進捗確認）。**★「会議室」は有璽氏の語。言い換えない。**
  - **★会議室 第2版（立ち絵入り・スマホ対応）を公開・同じURLへ積んだ**
    https://claude.ai/code/artifact/add7e2b8-968a-48c9-b551-2b8289f6a0a3
  - **🔴第1版は差し戻された**（有璽氏「ビジュアルが良くないわ。会議室っぽく全然ないねんけど」）。
    真因＝スクショを構成要素へ**分解**したあと、**自分の様式（見出し＋表）で組み直した**＝絵がゼロ。
    → [[feedback_read_the_artifact_not_the_copy]]「実物を見せられたら構成でなく見た目を返す」
  - **★担当14人の立ち絵を生成した（$1.86・有璽氏の許可あり）。**既存の agents/*.png は
    キャラではなく記号アイコンだったので使えなかった（実物を開いて確認）。
    ★置き場は**両機**の `~/.vivid-relay/assets/agents_portrait/`（原寸）と `/320/`（表示用）
  - **★ここで見つけた穴2件を記録済み**：①`close_without_answer()` は台帳を閉じない
    （status=open のまま・架空の判断待ちが後続を塞ぐ・**当方では未修正**）
    ②ログの日付書式がバラバラで grep 1本では稼働を測れない（mtime で覆した）
  - **★副産物の発見**：`progress_report` は今日「変化なしのため送らなかった」＝仕組みは正常だが
    **受け取る側から見れば進捗が見えない**。有璽氏の要件の裏付けになった
  - **★次の一手＝STEP1**（会議室を実データで自動更新・スマホ対応）。可逆・読むだけ。着手可
  - **★同じ対象に手をつけないでください**: `memory/project_ai_office_console.md` ／ 上のArtifact
    ★`web/kadoban/` と `~/.vivid-relay/dashboard_*.py` は**まだ触っていない**

  <details><summary>▸ 着手当初の調査メモ（要件確定前・経緯として残す）</summary>
  - 有璽氏が外部LP `ai-work-studio-lp.pages.dev/index-v5-ai-shain` を提示。**要件は見せ方でなく
    「中身がちゃんと動いているもの」＋「このサイトでも直接指示できる状態」。**
  - **★読むだけで進めた。**台帳・Notion・kintone・本番サーバーへは1文字も書いていない。
    書いたのは `memory/project_ai_office_console.md`（新規）・`memory/MEMORY.md` の索引1行・この欄。
  - **★実物を確認して事実を1つ訂正した** ── 提示されたのは**動画のスクショ**（プレイヤーの
    シークバー 01:14 が写っている・字幕焼き込み・LP本文も「画面デモ約1分40秒」と明記）。
    **「直接指示できる」部分は実物では示されていない**（社長席・掲示板は3枚とも同じ絵のまま）。
  - **★動画の芯は「1つの案件をリレーで仕上げる」。**うちのいまの形は14体が独立に持ち場を回す構造。
  - **実測：部品はほぼ全部ある。** lsu-editor の ブラウザ→Blob→mini 経路（9/10・実測8項目合格）を
    流用すれば、新規に書くのは①画面の投げる欄 ②キューを読んで `run_agent.sh` を叩く1本、だけ。
  - **★うちに無いのは3つ**：①進捗％（`claude -p` は返さない。偽の数字は出さない）
    ②成果物カード（出口ファイルの命名を揃えれば作れる）③掲示板＝指示の入口（#a9a97b で判断待ち）
  - 副産物として「見せる1枚」を publish 済み（実データのみ・LPと同じ構成）
    https://claude.ai/code/artifact/add7e2b8-968a-48c9-b551-2b8289f6a0a3
  - **★同じ対象に手をつけないでください**: `memory/project_ai_office_console.md` ／
    ask_hub `#a9a97b` ／ 上の Artifact。★`web/kadoban/` と `~/.vivid-relay/dashboard_*.py` は
    **まだ触っていない**（回答が来てから着手する）
  </details>

- **【ビビ / MacBook 2026-09-09 22時台】LIFE STAND UP ── ★今日の到達点と★有璽氏の判断待ち4件**
  ```
  ✅ 本番WordPress  ★テーマをインストール済み（★方式B・★有効化していない＝サイトは不変）
                    ★ライブプレビューで通し確認まで完了（トップ/IG/活動ブログ/地図/採用/フッター）
                    ★残る不可逆は「有効化して公開する」の1手だけ ＝ ★人が押す
  ✅ Instagram      接続→フィード作成→★本番プレビューで実表示。ショートコード=[instagram-feed feed=1]
                    テーマ側は do_shortcode + フォールバック（★デモは従来の枠のまま）
  ✅ デモ           005（お役立ち3分割）公開済み。恒久ルール①違反0件
  ✅ 仕組み         build_theme_zip.sh へ★受け取り手の要件検査(a/b/c)を追加＝同じ失敗が止まる
  ✅ 手順書         migration/本番への移し方.md の未確定 8件 → ★2件
  ```
  **★有璽氏の判断待ち（急がない・修正指示とまとめて）**
  ```
  ① ページの修正指示（多数・有璽氏が確認中）
  ② 恒久ルール②（パンくずより上）★64件/14ページ  A全部 / B見える8件 / C対象外と決める
  ③ front-page.php:778「Updated weekly · 週次更新」＝英日併記。様式として残すか
  ④ 余計な4プラグインの停止（手順書 migration/余計なプラグインの片付け.md は用意済み）
  ```
  **★同じ対象に手をつけないでください**: なし（当方は判断待ち・走行中0件）



- **【ビビ→クローバー／ロビン(mini) / 2026-09-08 13時台】福祉施設の困りごと 網羅リスト＋自社の手札 ── 走行中**
  - 有璽氏の宿題（2026-09-07 BPI研修・議事録48行目「福祉施設が一般的に抱える困りごとを整理しリスト化」）。
    **目的はリストそのものではなく、チーム編成の設計図のもと**（困りごと→解ける専門家→集める）
  - 議事録の原本 `~/Downloads/【研修】脳体制理論 BPI研修_講師：諸橋薫さん①.docx`（読むだけ・書き換えない）
    テキスト化したもの → `~/.vivid-relay/bpi_minutes_20260907.txt`（mini・130KB）
  ```
  ✅クローバー   社外の知＝困りごと ★118件（1周目30→2周目118・欠番0/重複0を2経路で確認）
                 実測 効く範囲 D50/A50/B11/C6/保育1 ・ 総合 S10/A21/B46/C41
                 ★申告の分類別件数表は合計124で本体118と合わない → ★ビビが数え直した値を正とする
                 出口 ~/.vivid-relay/fukushi_pains_clover.md
  ✅ロビン1周    社内の知＝手札18件（①提供中1／②できる8／③構想7）
                 ★Notion/Drive MCPがmini側に無く未参照。ローカルのみ → ★ビビが埋める
                 出口 ~/.vivid-relay/fukushi_ours_robin.md
  走行中ロビン2  ★クロス判定 118件 × 119番の業務内訳A〜E を ◎/○/△ で。PID 5451
                 △は「要る専門家」を資格名で → ★これがチーム編成の設計図になる
  ...
  **★同じ対象に手をつけないでください**: なし（作業完了）
  ...
  - **★補助金・助成金も対象**（有璽氏「ビビッドがこれまで取り組んできた領域」）＝7本目のメニューとして判定

  **✅第1版 公開済み（9/8 14時台・18:00予定を前倒し）**
  https://claude.ai/code/artifact/6f347967-ee9f-4d2b-b190-098920207cfe
  ```
  実測  困りごと 118件（欠番0・重複0を2経路）／ 判定 ◎11 ○13 △94（本体表を独立に数え直し）
        効く範囲 A50/B11/C4/D52/保育1 ・ 重要度 S10/A21/B46/C41
        ★S or A なのに△ ＝ 23件（最優先で人を集める場所）
        専門家別 中小企業診断士16／社労士15 → ★上位2者で31件（△の約1/3）
        ★中小企業診断士は議事録に一度も出ていない（実測で1位）
  補助金 看板は3本柱の1つ・実体は 案件0件/ナレッジ1件/商談3件。直接◎は#66の1件のみ
  ```
  - **★以後の版は同じURLへ積む**（作り替えない）。次＝9/9レビュー → 9/10 社外版（諸橋氏へ）→ 9/17 研修

  **🔴第1版は「粒度が粗い」と差し戻し（9/8 14時台 有璽氏）── 第2版を走行中**
  > 「一口にRPOと言っても何をやるのか分からない。具体性がない。**具体的にどの業務に対して
  >   人がいるのか**が分からない。**それを全部洗い出せ**。中身がなくない？
  >   **価格帯も参考値として入れて。掛け合わせの候補も。**」
  ```
  ★形    スプレッドシート（1行=1業務）＋ Artifactは足すもの
  走行中  ロビン    自社側の業務分解＋掛け合わせ5案以上  PID 9478 → f119_tasks_robin.md
          クローバー ★△94件から外部の業務を洗い出す＋価格帯  PID 9532 → f119_extern_tasks_clover.md
  ビビ    スプレッドシートを作って流し込む → Artifactへ積む
  列      業務ID/大分類/業務名(★動詞で終える)/何をするか/誰がやるか(★資格名)/自社可否/
...
- **【ビビ→ピタゴラス(mini)×3 / 2026-09-05 17:2x】miniが止まらないようにする ── 3層を並列で走行中**
  - 有璽氏「Mac mini側が止まらないようにしてください」＋「**対策については複数またがって用意して**」
  - **★触るファイルが重ならないようスコープを切って3体を並走させている。越えないこと**
    ```
    層1 検知   ~/.vivid-relay/stall_watch.py（新設）      走行中 → stall_watch_result.md
               走行中のclaude -pが「ログ更新時刻」で無言になっていないかを見る
    層2 予防   bin/hooks/ に PreToolUse を1本（新設）     走行中 → layer2_result.md
               非対話セッションのAskUserQuestionを止める。★settings.json登録は人の手
    層3 解除   ~/.vivid-relay/notify.py ＋ bin/hooks/self_audit.py  走行中 → layer3_result.md
               ask_hubのボタンで答えたらpendingが解ける形へ。★9/4の改修が巻き戻ったまま
    ```
  - **★同じ対象に手をつけないでください**: 上記3スコープ ／
    `hook_permission_slack.py`（既存の着手宣言を尊重し3体とも触らない）
  - **★3体とも「AskUserQuestionを使うな／.pyが書けなければサブエージェントへ渡せ」を指示文に明記済み**
  - 実測（2026-09-05 17:10 ビビ）：`notify.py` は ask_hub 0件・124行＝巻き戻ったまま ／
    `slack_pending.json` に今朝08:30の未回答が残存＝後続の通知を塞いでいる

- **【ビビ→ロビン(mini) / 2026-09-05 17:50】KPI数値設計 ⇄ テレアポリスト の整合を測る ── 走行中**
  - 有璽氏「**まずKPI等の数値設計とテレアポリストとの整合性が取れる状態にしたい。
    すぐにでも動き出す状況なので、やり方を営業側に伝える必要がある**」（★期限あり・中村氏は9/7から）
  - **★読むだけ。1文字も書かない**（台帳・Notion・kintone・シート・Slack すべて）
  - 測るもの：①🔴営業が明日架電に使えるのはどちらのファイルか（新リストにステータス入力欄が
    有るか無いか）②母数900 vs 7,646/3,271 と TBD-12「枯渇」の再試算 ③ステータス対応表が
    新リストで成り立つか ④「通知」に使えるいま在る経路 ⑤領域の重なり
  - 出口 `~/.vivid-relay/kpi_align_result.md`（★最後に「営業へ明日伝えられること／まだ伝えられないこと」）
  - **★同じ対象に手をつけないでください**: なし（読むだけ）。ただし新リストは別セッションが
    検査・修正中＝**版が動く。読んだ時刻とタブ構成を記録させている**

- **【ビビ / MacBook 2026-09-05 17:22】営業台帳の判断4点をSlackのボタンで発行 ── ✅4件とも回答済み（17:26〜17:27）**
  ```
  #5bbcb0 ① → 2 期限なしで保留にする        #a3a71e ② → 2 01からも同じ3行を消す
  #e509fe ③ → 2 AIの判断で変更してよい       #497cbd ④ → 1 まず中身を全部見せて
  ```
  **★②は推奨（00へ戻す）と逆のご判断。従う。ただし消える3行の全列を先に記録へ残させている**
  （8/31はそれをせず消して指摘が14→21組に増えた）。**②③は書き込み直前で止め、つる検査へ回す**

- **【旧・記録】判断4点の発行時の内容**
  - ロビンの引き継ぎ書 `scratchpad/営業台帳の指摘_引き継ぎ書_20260905.md` の論点1〜4を ask_hub で発行
    ```
    #5bbcb0  ① 毎朝の督促（8日連続）をどう扱うか      推奨＝期限を切って外す（10/31まで）
    #a3a71e  ② 消して片側だけ残った3件（正本の削除）   推奨＝00へ3行を戻す（控え実在）
    #e509fe  ③ 法人番号が存在しない行の顧客種別        推奨＝候補一覧を出して有璽氏が○×
    #497cbd  ④ 受付シート41行目（9日経過）            推奨＝まず中身を全部見せる
    ```
  - **★台帳・Notion・kintoneへは1文字も書いていない。**発行は mini から（台帳はminiにしかない）
  - **★投げっぱなしにしない。**次に動く回で必ず `ask_hub.answer_of()` を見に行く
  - ★②は正本の削除／③は値の変更＝**回答が来るまで台帳へ触らない**

- **【ビビ→ロビン／つる 2026-09-05】営業台帳の指摘（8日連続）── ★引き継ぎ待ち。サイト制作のセッションからは手を引きます**
  - 有璽氏「ここのセッションではスタンドアップのホームページ制作がメインなので、
    **それはまた別のところに引き継いで**くれたらいい」。★成果物は要る・扱う場所を移す
  - **できているもの**（読めば議論に入れます）
    `~/vivid-ai-hq/scratchpad/営業台帳の指摘_引き継ぎ書_20260905.pdf`（＋.md・両機に在り）
  - **★要点**：8日続く督促の最大の1件（重複21組）は、**8/31の「掃除」で14→21組へ増えていた**。
    消したのは行ではなく**判定に使われていた決着の記録**。控えは実在し、戻すことも可能
  - **★有璽氏は8/31に「掃除は置いといてよい」と決めている。仕組みがそれを知らない**のが真因
  - **走行中（mini・出口はSlack）**：つるが「台帳とテレアポリストの整合」を実測中
    → `~/vivid-ai-hq/scratchpad/台帳とテレアポリストの整合_20260905.md`（＋.pdf）に出ます
  - **★議論すべき論点は引き継ぎ書の6章に4点、選択肢の形で用意済み**
  - ★今朝08:30の通知が**記述式（ボタンでない）**で未回答のまま。後続の通知を塞ぐ構造

- **【ビビ→ピタゴラス(mini) / 2026-09-05】テレアポ営業リスト ── ★作った。つる検査中。まだ有璽氏へ渡していない**
  - できたもの https://docs.google.com/spreadsheets/d/1PXBlBraJrbDH0habkDeRGJT4U5NSRE03BsUXrT_ZbDA/edit
    13タブ・データ10,982行。共有ドライブ `13-000_営業部門/テレアポリスト`（見本と同じ場所）
  - ★有璽氏の決定（2026-09-05・確定）: 出力先＝**新規スプレッドシート**／大阪の基準＝**事業所住所**／
    複数サービス法人＝**「複数サービス」タブを別に立てる**／放デイ＝**含める（5種別）**。
    ★テレアポトークの種別ごとの書き分けだけ保留
  - **既存3本は無傷**（modifiedTime+version ／ タブ構成 の2経路で確認）
  - 🔴**ビビが実物を読んで実害1件を発見**：`活動FB` タブに**放デイの過去実績の集計行**が
    そのまま入っており、本来の見出し行が無い（作った側の申告は「見出しだけの空の器」だった）。
    ★A列で数えると0行に見えるので検算では出ない。**直すのはつるの検査が返ってから1回で**
  - つる検査中（`~/.vivid-relay/saleslist_tsuru.log`）。★検査中は作る側に触らせない（版を固定）
  - **★同じ対象に手をつけないでください**: 上の新規ファイル ／ 見本 `1x_KgLfPbreJQe8...` ／
    抽出元2本（`1p61-pY7viFY...` `1AWMNoofn3Vm...`）
  - ★人の判断が要る残り2点 ── ①名寄せで束ねなかった候補81件（法人名は同じだがキーが違う）
    ②テレアポトークの文言が放デイ前提のまま

- **【ビビ→ピタゴラス(mini) / 2026-09-05・判定フェーズ ✅完了】実現可能性の判定（読むだけ）**
  - 有璽氏の依頼「他サービス（就労A/B・移行・児発）も放デイと同じ形式で。大阪府に絞る。
    サービス種別ごと＋事業所単位/法人単位でタブを分ける。作れそうか確認して」
  - **★同じ対象に手をつけないでください**: 見本 `1x_KgLfPbreJQe8...` ／ 抽出元2本
    （`1p61-pY7viFY...` 事業所単位・`1AWMNoofn3Vm...` 法人単位）
  - **書き込みは一切していない**（3本とも読むだけ。新規スプレッドシートも作っていない）
  - ビビ実測: ★列見出しは見本と抽出元で完全一致＝フォーマットの作り変えは不要／
    ★抽出元に放デイは0件（ソースが別）／★法人単位の種別はカンマ連結で種別タブに重複する
  - 判定と設計は mini のピタゴラスへ委譲（`~/.vivid-relay/saleslist_pitagorasu.log`）
  - 詳細 → `memory/project_telapo_list_other_services.md`
...
- **【ピタゴラス / MacBook 2026-09-05】zaimu_classify.py 新設 ── ✅実装・本実行完了。要判断1点**
  - ビビ依頼（MacBookスリープで長時間処理が落ちる対策・機械判定はスクリプト化してminiへ切り離す）
    ...
    **台帳・Notion・kintoneへは1文字も書いていない**（Drive上のCSV+Sheetsのみ・読むだけ元ファイル）
  - **★対象は「2026年5月の対応表」との摘要完全一致（LLM判断なし）**。一致しないものは
    推測で埋めず「その他経費/未分類(要確認)」に残す設計
  - **実装済み口座**: ILIFE/SWELL/ビビッド楽天銀行(全銀協形式)・個人楽天銀行(★4列・全銀協とは
    別形式と実測で判明・前方一致ロジックで対応)・個人りそな銀行(21列)・個人PayPay(13列)・
    個人Amex(6列)
  - **★対象外（未実装・理由をログに明記）**: ILIFE_関西みらい銀行PDF
    （pdfplumberで実測したところ抽出テキスト0文字。過去セッションが使った
    Google Driveのread_file_content的なOCR変換はmini上の自律スクリプトからは呼べない）／
    個人_楽天カード（有璽氏の指示で対象外）
  - **本実行済み（2026-08分）**: 401件 → CSV`判定結果_2026-08.csv`(id
    1u5NkwNzUvlkPdXK6WMPLTDjQ40HF933K)＋スプレッドシート`予実 確認シート 2026-08`(id
    1B8jENnyVpKAss-bxhJL5iBO580f56C2iOnuP-2XR0R4、シート1=要確認185件/シート2=全件401件)。
    確信度 高216/中25/低160。実物2経路(メタデータ・CSV内容)で作成を確認済み
  - **🔴重複を発見・報告のみ（当方は何も統合していない）**: 着手前に確認したところ、
    2026-09-04付で別セッションが既に銀行5口座156件ぶんを手動処理し
    `判定結果_2026-08_銀行.csv`＋同名スプレッドシートをDrive上に作成済みだった
    （project_cfo_agent.mdに記録あり）。今回のファイルは名前が違うため両方が並存している。
    **★9/4版は対話的な手動処理で再利用不可、今回のは--month引数で毎月使えるスクリプト**という
    違いはあるが、**8月分については同じ目的の成果物が2つ存在する状態**。統合・削除は
    正本の書き換えにあたるため当方では行っていない。有璽氏かビビの判断が要る
  - **PayPayの小口決済**（日用品を月合計1行にまとめる設計）は**未実装**。今回は個別行のまま
    出力しており、これが未分類率(40%)の主因の一つ
  - 依存: 両機に `openpyxl` `pdfplumber` を pip install 追加（mini・MacBook両方で実測確認）
  - 次回9月分以降は `python3 ~/.vivid-relay/zaimu_classify.py --month 2026-09` で同じ経路が使える
  - cron/daily_jobs登録は行っていない（今回は単発実行の指示のため）
...
- **【リリス / MacBook 2026-09-05】見学・お問い合わせフォーム20項目 CF7貼付テキスト＋PDF説明書 ── ✅完了**
  - **成果物**（`~/lifestandup-wp/proposal/`・commit `92080ae`）:
    ...
    ②`見学お問い合わせフォーム_説明書_20項目確定版.md`＋`.pdf`（並び順の理由・選択肢・確認事項）
  - **本番サーバー・WP管理画面・メール送信は一切触っていない**。テーマのPHPも1行も変更していない
  - 郵便番号→住所自動補完は有璽氏の承認で案A（zipcloud公開API・JS）を採用。実装はCF7の
    「フォーム」タブ内へscriptタグを含める形（テーマのPHP改修なし）。失敗・該当なし・
    通信失敗・既入力ありの4状態すべてで案内文を必ず書き換える（サイレント失敗にしない、
    有璽氏の追加指示に対応）
  - **実測で確認済み**：①zipcloud API実疎通（HTTP200・CORS許可ヘッダーあり）②ローカル検証用
    WordPress（`_tools/`・PHP8.3.32/CF7 6.1.7）へ一時テストフォームを作成しCF7構文20項目が
    構文エラーなく展開されることを確認（確認後は削除・本番未使用）③日付欄`min:today`が
    実際に本日日付(2026-09-05)へ動的展開されることを確認④JSロジック単体テスト（Node.js・
    DOM/fetchモック）5ケース（成功／該当なし／通信失敗／既入力あり／桁数不足）全合格
  - 既存の旧提案 `proposal/見学お問い合わせフォーム_項目突合とCF7案.md` は20項目確定により
    判断事項が解消したため降格注記を付けて残した（削除していない）
  - **有璽氏への確認事項4点**（詳細はPDF説明書8章）：①学年「その他」選択肢を残すか
    ②メール確認用の一致検証はテーマPHP改修が要るため見送った・追加してよいか
    ③フォームID`7b81a8b`へ上書きか新規フォームか ④CF7の「メール」タブ設定は別途要
  - **⛔2026-09-05追記：検査で実害バグを1件発見・修正済み（commit `8acc0f5`）**。
    「ご希望の連絡方法」「受給者証の有無」がラベル〈必須〉なのに`[radio ...]`に`*`が無く
    空で送信できた。★単純に`[radio* ...]`へは直していない（実測：CF7に`radio*`という
    タグ自体が存在せずフィールドが消えることを確認）。`[checkbox]`+`exclusive`オプション
    へ置き換え、*の有無で必須/任意を正しく制御できることを実測確認。あわせて自己発見2件も
    修正：①「お子さまの性別(任意)」もradioの仕様上*の有無に関わらず常に必須化していた
    ②個人情報同意の`optional:false`はCF7が値を見ず「optional」の文字列だけで判定するため
    逆に任意になる実装ミスがあった（optional指定を削除）。修正後、必須14・任意6の
    計20項目を1つずつ空にして送信するテストを実施し全項目が正しく機能することを実測

- **【リリス / MacBook 2026-09-04】LIFE STAND UP フォームをCF7へ接続 ── ①暫定接続は完了。②③は方針転換で保留**
  - ①見学・お問い合わせを本番CF7 `7b81a8b` へ暫定接続 ✅commit済み（654b196）。
    CF7 5.8以降のフォームIDはハッシュ文字列という実装差を発見・
    `lsu_cf7_form_id()`をint→stringへ修正（`contact.php`のintキャストも1行修正、
    実測: (int)"7b81a8b"=7に壊れることを確認）
  - ②③（見学問合せ・採用応募フォームの中身）は有璽氏の方針転換により**保留**
    （commit 01d09b3）。「項目をゼロから設計し直す。他社事例を調査したい」との指示。
    採用応募フォーム手順書は項目表を外した骨格版のみ完成。
    既存CF7 2本の項目突合・日付固定(`min:today`で動的化可能と実測済み)・
    別サービス「ライフ スタディ」選択肢混入の材料は
    `~/lifestandup-wp/proposal/見学お問い合わせフォーム_項目突合とCF7案.md` に保存
    （次の設計担当への申し送り用・実装はしていない）
  - **★同じ対象に手をつけないでください**: `~/lifestandup-wp/theme/lifestandup/functions.php`
    （`lsu_cf7_form_id()`周辺）
  - **★①のCF7暫定接続を残すか元のデモUIへ戻すか未確定**（設計未定のまま残っている）。
    次に触る人は有璽氏の判断を確認してから触ること
  - **⛔2026-09-05更新（リリス）**：②③の保留（「項目をゼロから設計し直す」）は、
    有璽氏が20項目を確定させたことで解消。続きは上の「【リリス / MacBook 2026-09-05】
    見学・お問い合わせフォーム20項目 CF7貼付テキスト＋PDF説明書」ブロックで着手中。
    **`~/lifestandup-wp/proposal/`配下の「同じ対象に手をつけないでください」指定はここで解除**
    （新規2ファイルのみ追加。旧提案mdは降格注記を付けて残す・削除しない）

- **【ピタゴラス / MacBook 2026-09-04】判定結果CSV(156件)をGoogleスプレッドシート化 ── 着手**
  - 対象: `判定結果_2026-08_銀行.csv`(id `1D1ZQIxDIjwfamfTak2m2CyuHBoi0bD5a`) を
    新規スプレッドシート「予実 確認シート 2026-08」へ作り替え（月次明細フォルダ配下）。
    有璽氏が○×を付けられなかった(CSVは編集の器にならない)ことへの対応。ビビ依頼。
  - **元CSVは消さない。共有設定は変えない。** シート1=要確認(絞り込み)／シート2=全件156件
  - **★同じ対象に手をつけないでください**: 上記CSV／新設スプレッドシート／月次明細フォルダ

- **【ピタゴラス / MacBook 2026-09-04】予実 判定結果ファイル(銀行5口座156件) ── ✅完了。ビビへ報告済み**
  - **今回の対象は銀行5口座156件のみ**（PayPay・Amex・関西みらい・楽天カードは対象外・触っていない）。
    ILIFE_楽天銀行53件／ビビッド_楽天銀行26件／SWELL_楽天銀行46件／個人_楽天銀行15件／
    個人_りそな銀行16件＝156件、依頼どおりの内訳と実測で完全一致
  - **成果物**: Drive月次明細フォルダ配下に `判定結果_2026-08_銀行.csv`（新規）
    id `1D1ZQIxDIjwfamfTak2m2CyuHBoi0bD5a`。1行1取引・列＝日付/法人/口座/摘要/金額/勘定科目/
    事業/判定の根拠/確信度。**Notion・元CSV/PDFへは1文字も書いていない**（読むだけ）
  - **★検算で一致（重要）**: ILIFE合計+414,869円、SWELL実支出-1,142,698円（+900,000振替を除く）、
    ビビッド実支出-956,473円（+950,000振替を除く）が、いずれもNotion予実管理DB27行の
    8月確定速報と完全一致。全銀協形式(和暦YYMMDD)のパースとwareki_to_seireki変換が
    正しく機能していることの裏付けになった
  - **★りそなCSVの手動転記で2回データ欠落が発生した（正直に報告）**: base64が反復性の高い
    構造（同一ヘッダー19列が116行すべてで完全一致）で、2回とも数百バイト分silently欠落。
    3回目に read_file_content（文字化けするが数字は正確）でテンプレート再構築する方式に
    切替え、8月分16件を正確に確定（既知16件と一致）。摘要の漢字部分が判読不可の2件は
    「文字化けで判読不可・原本要確認」と正直に未分類にした。**大きな反復データをLLMで
    手動転記しない**という既存の教訓([[project_cfo_agent]])が今回も再現した
  - **未分類件数（法人別・金額付き）**: ILIFE 9件/2,131,929円（うち国保連+3,192,297円を含む）／
    SWELL 9件/-719,486円／ビビッド 1件/-20,700円／個人 21件/2,579,077円（うち国保連
    +3,011,748円を含む）。★未判定率は件数と金額で逆転する点に注意
  - **有璽氏に見ていただく候補（アップロード済みCSV内に記載済み）**:
    ① ILIFE未判定の金額大きい順（国保連3,192,297円・600,000円ILIFE自身名義の出金で
    対応する入金が見当たらないもの・-122,451円ノムラケント等）
    ② 給与手当と判定した1件（ビビッド-77,000円 エムズデザイン下村メイ・根拠薄いとビビ既述）
  - **★残1点（有璽氏かビビの操作が要る）**: 検証中に誤って作成した一時コピー
    `_tmp_risona_2601-2608_convert`（id `14Cv23FZ0h3G6Tuc-7o-M3dbDc3nez7ST`・個人_りそな銀行
    フォルダ内）を trash_file で消そうとしたが権限拒否（don't ask mode）。中身はりそな原本の
    単純コピー（19,194バイト・機微だが実害は無い）。**削除をお願いしたい**
  - scratchpad内の一時ファイル（口座番号・取引先名・金額を含む中間データ）はすべて削除済み
  - 下の「【ビビ→ナミ→つる / MacBook 2026-09-03】予実の設計」ブロックの続き（集計は完了・
    その根拠ログが無いという指摘への対応）として実施した
...
- **【ビビ→ナミ→つる / MacBook 2026-09-03】予実の設計 ── ★器は完成。有璽氏の材料待ち**
  ...
  - ✅ **9/4 区分列を27行とも投入済み**（売上2／販管費24／その他1・空欄0を実測）
  - ✅ **9/5 判定スクリプト `~/.vivid-relay/zaimu_classify.py`（両機・sha256一致）を mini で本実行。
    判定結果 401件 ＋ 確認シート**（要確認185件）
    https://docs.google.com/spreadsheets/d/1B8jENnyVpKAss-bxhJL5iBO580f56C2iOnuP-2XR0R4/edit
    **★9/5 現在 有璽氏が○×を記入中。**埋まったら読み取って反映する（★次月から同じ判定が自動で効く）
  - **★9/5 恒久の運用を決定** → [[reference_offload_long_work_to_mini]]／fukuchi-core にも原則を記載
    MacBook＝議論の場／mini＝実作業の場。**長い処理は ssh mini + nohup で投げる**
    🔴`VIVID_NOTIFY_OFF=1` は ssh 越しに渡らない。**コマンド文字列の中に書く**
  - **🔴 まだ入っていない**: 関西みらい19件（PDFからスクリプトで読めず）／楽天カード13件（中身が2025年8月分）
  - **🔴 有璽氏の判断待ち**: ①gitの履歴に残った口座番号（may_rows.pkl・push済み）を消すか
    ②同じ8月の成果物が2つ並存（9/4の手作業156件版と9/5のスクリプト401件版）
