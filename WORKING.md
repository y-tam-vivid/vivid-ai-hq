...
### 【ビビ 2026-08-29】記憶が届く経路を直した ── ★実装・実測済み。残＝MacBookへの配布

有璽氏「散々書きますと言っててもそれ見ないんやったら意味ないやん。根本的にどうやって解決すんの」。
**調べたら、届ける仕組み自体が3か所壊れていた。** 詳細 → `memory/reference_no_gate_on_asking_the_human.md`

```
① 人へ投げる経路だけ無検問     Stopフックに検査2を追加。未検索で「無い/いただけますか」→差し戻す
② 在庫が188本中100本を落とす   直近7日以内は上限に関わらず確保。常設枠 19本 → 59本
③ 出すのが上位2本に固定        まだ出していないものを先に出す（5回連続で別々に出ることを実測）
★ build_landmine_index.py が NameError を出しながら exit=0 → 落ちたら 1 ＋ 心拍「失敗」へ
```

- **触ったのは `bin/hooks/{hook_session_writeback,hook_inject_memory,build_landmine_index}.py` の3本のみ。**
  台帳・Notion・kintone へは1文字も書いていない
- 配布済＝mini の `~/.vivid-relay/`（settings.json が呼ぶ実体）。**★MacBook は未配布**
