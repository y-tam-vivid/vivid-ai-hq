...
### 【ドーベルマン 2026-08-31】台帳の掃除 ── 汚れの棚卸しと「直す価値」の判定（読むだけ）

つるの成果物（重複16組・法人番号空58件）を土台に、決着済みを除いた実数と直す価値を判定中。
`ledger_dupes.py` `ledger_report.py` `corp_number_fill.py` を読むだけで実行。
★intake系（intake_notify/intake_match/intake_register）には触らない。台帳へは1文字も書かない。
