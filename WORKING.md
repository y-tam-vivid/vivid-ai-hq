...
- **`08_関係フォロー`** … シート本体と自動入力は**構築済み・稼働中**。
  残るは仕上げの ㉗v2 のみ（**ドライラン待ち・まだ1バイトも書いていない**）
  - 済: 14列のシート新設（㉕）／`onRelationEdit` 設置・動作確認（㉖）
  - 残: ㉗v2 ＝ ①09に⑤ブロック追記 ②ゴースト行の条件付き書式 ③95に運用ルール3行
    ④09に残る旧名「フォーム営業」→「問い合わせ営業」
  - Drive `1QZHs-bPPvw5EIU8jGE636SiJfTLyrYatPZB3kO0NMGA`（v1は「【旧版・使わない】」へ改名済）
  - **本実行の前にバックアップ**（AIが自律で書くため）
