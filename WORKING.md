...
### ★人の手が要る3件（これが無いと先へ進めない）

| # | 何を | これが無いと |
|---|---|---|
| 1 | Notionインテグレーション「Chatworkリレー」を **🏢顧客DB** へ接続（⋯→接続） | 毎朝08:00のNotion同期が作れない（API 404・2026-08-20実測） |
| 2 | 国税庁Web-APIの**アプリケーションID申請** | 法人番号の自動補完ができない。要判断17組の大半がこれで片づく |
