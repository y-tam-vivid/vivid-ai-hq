...
- **【リリス / MacBook 2026-09-03】有璽氏の回答①〜⑤への対応 ── ✅⑤の画像・管理画面手順・適用計画まで完了**
  - **書いた**: `~/lifestandup-wp/proposal/` のみ。**`theme/` と `verify/` には1文字も触っていない**
    （ステラが検査中のため。適用は検査が返ってから一度に行う）
  - 渡すもの4点（ビビ経由）: `bridge-4up.png`（⑤4案の比較）／`recruit-banner-mock.png`（①）／
    `font-compare.png`（③）／`管理画面で見てほしいこと.md`（有璽氏が見る場所を6項目で特定）
  - **②はリリスが決定**（有璽氏から委譲）: **★共通CSSは作らない。** 全20本で一致するのは21セレクタだけ。
    色トークンだけcommonへ／残りはページ別に閉じる＝崩れない。**工数 62〜112h → 52〜95h**
  - **★訂正2件**: 前報「デザイントークンが揃っていない」は言い過ぎ（色は全20本一致・違うのは書体1個）／
    `--dump-dom` は `--window-size` を効かせる（前報の測定幅の記述が不正確だった）
  - **★見た目を変える変更（①③④）を当てたら、再現度の実測をやり直す**（前回の「差分0」は無効になる）
