...
### 【ピタゴラス / mini 2026-09-05 夕】走行中の claude -p が無言で止まったことを検知する役を新設 ── 着手（書く）

**書くのは `~/.vivid-relay/stall_watch.py`（新設）と `~/.vivid-relay/stall_watch_result.md` だけ。**
既存ファイルは読むだけ（触るなら先に `_backups/` へ控え）。
**台帳・Notion・kintone へは書かない**（⚙️レジスタへの心拍1行のみ例外）。
**Slackへ実投稿しない**（`notify._post` と `ask_hub.api_post` の両方をモック）。
**cron/daily_jobs へ登録しない**（手順だけ書き残す）。**走行中の作業を殺さない**（検知のみ）。
