...
- **【リリス / MacBook 2026-09-12】JFBI 調査研究サイト（1ページ・静的）を実装しVercelへ ── 着手**
  - 入力 `~/Downloads/発達特性とお金の調査研究サイト/`（**読むだけ。1バイトも書き換えない**）
  - 出力 `~/jfbi-research-site/`（新設・repo外）。index.html / style.css / script.js / robots.txt / assets
  - 正本＝`design/Research Site.dc.html`（プロトタイプ）。★DS既定と README が食い違う3点は
    **README を正**とする（README が「施主判断で本文15px・lead16px・ボタン14px」とDS既定17/18pxを明示的に否定）
  - **★実測で確定した2点**：①SVGロゴはライブテキスト（font-family指定あり）＝閲覧者環境依存なので使わない。
    READMEの指定どおりPNGを使う ②`sips -s format webp` は**パスを返すがファイルを書かない**（無対応）。
    WebP変換は Pillow 11.3.0（webp:True）で行う
  - **触らない**: 台帳・Notion・kintone・Slack／他案件（lifestandup-wp・かわちばなし）
  - **★同じ対象に手をつけないでください**: `~/jfbi-research-site/` ／
    `~/Downloads/発達特性とお金の調査研究サイト/`（読むだけ）
