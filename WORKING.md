...
### 【ピタゴラス 2026-08-29】Word/PowerPoint/PDF/ffmpeg 環境整備 ── ①🔴構造的に本体反映不可・②③④ステラ検査中
...
② bin/md2pptx.py 新規      ✅完了・両機実測OK。ステラへ検査依頼中
③ bin/md2pdf.py 新規       ✅完了・両機実測OK。ステラへ検査依頼中
④ bin/setup_ffmpeg.sh 新規 ✅完了・両機実測OK（reelモードのffmpeg PATH依存も修正・両機で
                          h264動画生成を確認）。ステラへ検査依頼中
```
...
### 【ピタゴラス 2026-08-29】古い写しの実物合わせ・伝播検知の仕組み ── 着手中

有璽氏「言ったことが通ってなくて、上書きもされていない状態を解消しろ」を受けた対応。
①`.claude/skills/customer-db-sync/references/targets.md` を実物(kintone実測)に合わせる
②同じ事実が複数箇所にあり片方だけ古い状態を機械で見つける仕組み。
対象: `.claude/skills/customer-db-sync/references/targets.md`（新規） ／
`bin/hooks/self_audit.py` か新規スクリプト（検討中）。台帳・Notion・kintoneへは書かない。
