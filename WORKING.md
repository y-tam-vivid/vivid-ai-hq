...
- **【ビビ / MacBook 2026-08-22】 進行中 ── GameBull（ゲーム型販促PF）を SalesBreaker で新規開拓**
  - 対象：**新商材のターゲット設計＋フォーム営業文面（書くのは設計物のみ）**。
    営業ワークブックは**読むだけ**（NGリスト用に社名/URLを吐く）。台帳へは1セルも書かない
  - 送信は未実行。**SalesBreakerへの投入・送信は有璽氏の承認後**
