...
【ピタゴラス着手中 2026-08-20】`notion_customer_upsert.py --restore` の実測検証（つる指摘：復元側PATCHは未検証）。
Notion🏢顧客DB内に検証専用ページ1件を新規作成して壊して戻す。**既存486件へは書かない**。終わったら消す

**設計図（目指す姿）** → Notion 🎯 `3c17b1568b5781aab657cad9e56bdb7f`

### 【ビビ着手中 2026-08-20 02:50】法人番号の一括補完（経路B）── **台帳へはまだ1件も書いていない**

- **現在地**: つるが `corp_number_fill.py` 自体を直した（02:48）── ④閉鎖済み除外を候補数に
  関係なく通す／`decide()` に「登記記録が閉鎖」「根拠不足」「都道府県・市区町村・番地の
  食い違い」「所在地が台帳に無ければ確定しない」を追加。壊れていたキャッシュを消し
  **`--rescan` で581万件を引き直した**（02:49・走査5,816,535行）＝**確定4件／要判断79件**。
  **つるへ再検査に出した（走行中）。返るまで `--apply` しない**
- **★確定の中身が総入れ替えになった。**前回の8件のうち残ったのは B-0084 だけ。
  つるが「書いてよい」と通した B-0372／B-0373／B-0391 も落ち、
  前回1件も出ていなかった B-0035／B-0130／B-0367 が新たに上がった。
  **原因はキャッシュ欠落（TSV87行に対しキャッシュ83行）＝「確定」の根拠が再現できない状態で
  判定していた。**「確定が減った＝安全になった」とは限らないので、両方向を検査に出している
- 今回の確定4件 ＝ 36 B-0035 Lien／85 B-0084 合同会社T.O.／131 B-0130 SPC／368 B-0367 ユナイト
  （B-0084 以外は**番地まで一致**）
- **対象**: `00_企業マスタ` の法人番号欠損 ─ **読む → 突合 → つるの検査 → 書く**
- 突合元は既にローカルにある `~/data/houjin_bangou/00_zenkoku_all_20260731.csv`（581万件）
- **つるの検査 完了（02:44）。★このまま `--apply` してはいけない**
  - 「確定14件」のうち**書いてよいのは4件だけ**（85 合同会社T.O.／372 ジャパンネットワークグループ／
    373 合同会社ウィル／377 日本アクセス）。いずれも**番地・ビル名まで一致**＝2経路
  - 書くなが9件（都道府県が無関係／台帳が全項目空欄で社名1経路のみ）＋統合済みの廃止行1件（142 B-0141）
  - コードの欠陥3点 ①所在地の裏付けが0でも「全国1社なら確定」②**台帳の住所列を読んでいない**
    ③統合済みの廃止行を除外していない → **ピタゴラスが修正中**。直ったら再度つるへ通す
  - 住所列を足しても新たに自動確定できるのは5件だけ（Lien/SEcond/SPC/マーベリック/ユナイト）。
    残りは国税庁側にヒットしない53件が主で、**社名欄の汚れが真因** →
    `memory/reference_ledger_name_blocks_corp_match.md`
- **★検査を頼むときはTSVを直前に再生成する。** 渡したTSV(02:34)とコード(02:35)が別版で、
  つるが誤った土俵で判定しかけた
...
| 何 | 状態 |
|---|---|
| ~~移送54v3の毎朝トリガー化~~ | ⛔**やってはいけない残件だった**（2026-08-20 に判明）。54v3は会社名で突合して**機械が「新規」と判定して台帳へ書く**＝2026-08-19 に B-0050 と C-0072 を二重に作った経路そのもの。毎朝自動で回すと規範「機械が新規と判定して書いてはいけない」に正面から反する。**後継は Python 側の2本** ： `intake_match.py`（照合だけ・書かない・cron 07:20 稼働中）＋ `intake_register.py`（人が○を付けた行だけ登録・つる検査中）。★54v3 は手で実行する道具として残すが、**自動化しない** |
| フォームに「相手の種別」を1問足す | 法人／個人（事業主）／個人。ALLE＝個人事業・YJAPAN＝法人が判別できない |
| 質問順を 会社名 → 氏名 へ入れ替え | 逆転対策 |
| 法人番号の自動取得 | ★**申請は要らなかった。**全件データ（581万件・1.26GB）が `~/data/houjin_bangou/` に2026-07-31から在る。【ピタゴラス着手中 2026-08-20】突合して候補CSVを出す（★台帳へは書かない） |
