...
### 【リリス / mini 2026-09-06】区分・リンク・フォント・地図（A/B/C）── 着手（書く）

**★大前提が変わった（有璽氏 9/6）：仮公開サイトの見た目が正。**ここを直せば本番の設計になる。
原本HTMLとの1px差は以後、不具合として扱わない。

対象は `~/lifestandup-wp/theme/lifestandup/` と `static-preview/`（＋仮公開の出し直し）。
**本番サーバー・WP管理画面・DNS／台帳・Notion・kintone へは書かない。**
`middleware.js` は外さない。CSSを触ったら `check_css_braces.py` を通す。

```
A（実装する）  A-1 区分に「お役立ち情報」を新設（フロー3＋ストック1・その他は作らない）
               A-2 トップの活動ブログ3枚にリンク＋実データ化（★カテゴリ表記は原本の飾り）
               A-3 Instagram を life.standup_official へ統一（本文2箇所が誤り）
               A-4 --font-hand の二重定義を1つへ（★トップの見た目が正）
               A-5 X（旧Twitter）の共有ボタンを外す（アカウントが無い・LINEは残す）
B（簡易版）    B-1 地図にピン（緯度経度）  B-2 白黒化（★本式=地図APIは課金なのでやらない）
               B-3 トップに地図を出す
C（案だけ）    C-1 Instagram実投稿の取り込み（★プロアカウント確認中・実装しない）
               C-2 問い合わせ導線をLINEへ（★いきなり電話は現場負担）
```
