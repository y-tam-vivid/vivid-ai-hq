...
### 【ピタゴラス 2026-08-29】Word/PowerPoint/PDF/ffmpeg 環境整備 ── ②③④完了・①はdiff提示のみ(承認待ち)

有璽氏「いります。使います。使えるようにして」を受けた対応。詳細・実測値は
`memory/project_event_skills_suite.md` 冒頭「★2026-08-29 ピタゴラス着手」を参照。

```
① snspipe.pyフォント対応   ★diffのみ提示。ビビの承認待ちで未着手（本体は1バイトも変更していない）
② bin/md2pptx.py 新規      ✅完了。python-pptx両機導入・実測OK
③ bin/md2pdf.py 新規       ✅完了。reportlab(CIDフォント)採用・実測OK
④ bin/setup_ffmpeg.sh 新規 ✅完了。evermeet.cx静的バイナリ＋Rosetta自動導入・両機実測OK
```
