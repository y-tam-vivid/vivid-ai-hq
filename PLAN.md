...
## 13. 正本へ追加した規範 2件（**2026-08-13 本人承認・反映済み**）

**規範の変更は要承認**（正本「迷ったときの既定」）。2件を束ねて諮り、**両方承認され `fukuchi-core` へ反映済み**。
草案1は §4「どう進めるか」へ、草案2は §1「何を自分で決め、何を人に渡すか」の可逆/不可逆の直後へ追加した。

### 草案1 ─ 実行の入口は名前で判断しない

> cron・メニュー・手順書に載せる前に、その1行が「書くのか、書かないのか、壊すのか」を
> 実測で1回確かめる。名前からは判断しない。

根拠と4つの型は [[reference_dangerous_entrypoints]]（実測済み）。
この規則の妥当性は個別4項目の検証結果に依存しない。

### 草案2 ─ 不可逆な操作の前に「先に復帰経路を作る」

現行の正本には「可逆かどうかで線を引く／不可逆なら**承認を得る**」はあるが、
**「先に戻せる形を作る」が無い**。ただし**一律に書くと空回りする**ため、操作を2つに分ける。

```
復帰経路を作れる    データの変更（シート・DB・ファイル・コード）
                    → 先にコピーを取る。ここは必須にしてよい

復帰経路を作れない  送信・公開・課金・締結・署名
                    → コピーという概念がない。取り消せないものは取り消せない
                    → 従来どおり「承認を得る」のまま
```

**さらに「バックアップを取った」と「復帰できる」は別物**という一文を入れる。

- **実例（2026-08-13）**: 営業ワークブックの `99_バックアップ` を見て「8/10（日）の分が無い＝
  自動バックアップが停止している」と報告した。**これは誤り。8/10は月曜。** 日曜は 8/02 と 8/09 の
  2回だけで、**2回とも取れており取りこぼしはゼロ**（次回は8/16）。手順書の「トリガー未設置の疑い」も
  実データが否定した。誤った側の報告が、この草案の根拠として一度流通した。
- **そこから得た教訓（こちらを根拠にする）**:
  **止まっているものを見逃す**（間隔を持たない）と、**動いているものを止まっていると誤判定する**
  （在庫しか見ない）は、**同じ欠落の裏表**。どちらも「期待間隔がデータとして存在しない」ことから来る。
  手順書に文章で「日 19:50」と書いてあっても、曜日は人の記憶で計算され、間違う。
  → **稼働確認は「最後にいつ動いたか」と「次はいつ動くはずか」をセットで持つ。**
  片方だけでは両方向に間違える。「取った」と「戻せる」が別物であるのと同じく、
  **「在る」も「無い」も、それ単独では稼働の証拠にならない。**

### 経緯
ユーザー指示が同日中に転換した（「営業ワークブックは触らない」→「触ってよい。その代わり
誤操作しても復帰できる状況にして」）。趣旨は「触らせない」ではなく「壊れても戻せる」。
`memory/feedback_sales_workbook_hands_off.md` の更新は別セッションが担当（本セッションは触らない）。
