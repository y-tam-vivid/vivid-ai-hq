...
## 検査4（担当を通さず一人で実装）── 2026-08-31 追加

`hook_session_writeback.py` の `check_solo_implementation()`。
**実測は `bin/hooks/test_check4.py` で再現できる**（check.sh 項目9 から毎回叩かれる）。

| # | 書き方 | 捕まえるか | 実測 |
|---|---|---|---|
| 1 | `Edit` / `Write` で bin/・.claude/ の実装ファイル | ○ | 17件テストで確認 |
| 2 | heredoc `cat > path <<EOF` | ○ | 〃（当初はすり抜けた） |
| 3 | リダイレクト `>` `>>` | ○ | 〃 |
| 4 | `sed -i` | ○ | 〃（当初はすり抜けた） |
| 5 | `tee` | ○ | 〃 |
| 6 | `python -c "open(p,'w')"` | ○ | 〃 |
| 7 | `cp` / `mv` で配置 | ○ | 〃 |
| 8 | `awk` / `perl -i` | **×** | 未対応。すり抜ける |
| 9 | `xargs` 経由の書き込み | **×** | 未対応 |
| 10 | 変数展開したパス（`$F` など） | **×** | 未対応。静的には解決できない |

**★8〜10 は捕まえられない。** 検出型の限界であり、隠さず書いておく
（`reference_make_it_impossible_not_detectable.md` ── 1975年に結論が出ている型）。

**★当初の版は 1 しか捕まえられず、しかも `'team_run' in cmd` の文字列一致だったため
`grep team_run` で素通りできた。** チーム検査（つる・ステラ・ピタゴラス）が3者独立に
指摘して発覚した。作った本人は「5ケースで検証した」と報告していた。
