...
3点
  ...
  ★2026-08-29 ステラ検査（fresh eyes Pass B）で発見された死角・未解消のまま明記する：
    sys.path.insert(0, HERE) の HERE は「このスクリプトが実際に置かれているディレクトリ」を
    指す。つまり `bin/hooks/action_catalog_check.py` を実行すればgit repo側のモジュールを、
    `~/.vivid-relay/action_catalog_check.py` を実行すればそちらのコピーをimportする ──
    **どちらのコピーを実行するかで、見るLOG定数が変わる**。検査時点ではbin/hooks/側と
    ~/.vivid-relay/側がバイト一致していたため実害は無かったが、**将来git repo側だけを
    直して~/.vivid-relay/への複製を忘れると、この検問自身が「正本でない方を見て✓を出す」
    ＝穴Aと同じ型を再現しうる**。「穴Aの型を作らない」と明記していただけに、この一点が
    死角になっていた（ステラの言葉を借りる）。
    ★対応：コード修正はせず、運用ルールとして明記する（bin/hooks/*.pyを編集したら、
    その回のうちに ~/.vivid-relay/ への複製を diff で確認すること。今回もこの後の配布時に
    diff -q で一致を確認している）。
