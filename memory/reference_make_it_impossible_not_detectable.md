...
## ★★対策は1層で出さない ── 「複数またがって用意して」（2026-09-05 有璽氏）

**ビビが「miniが止まらないように」へ、検知（層1）だけを実装に回し、
予防（層2）と解除（層3）を「次の一手」と書いて後回しにした回。**

> 「**対策については複数またがって用意して。**」

```
ビビが出した形     層1 検知 ← 実装中     層2 予防 ← 「次の一手」    層3 解除 ← 「巻き戻ったまま」
有璽氏が求めた形   層1・層2・層3 を★同時に用意する
```

- **★検知だけ入れても、止まる回数は1回も減らない。** 検知は「止まったあと」に効く。
  このファイルの主題（検出型は原理的に不完全）そのもので、**検知を先に入れること自体は
  正しいが、それで完了にしてはいけない。**
- **★層を並べて示せた時点で、後回しにする理由はほぼ無い。** 層が見えているのに1つしか
  着手しないのは、優先順位ではなく**着手の分割をサボっている**。担当は並列で走らせられる
  → [[feedback_use_the_team_not_alone]]（ビビは投げて束ねる）。
  ★**触るファイルが重ならないようにスコープを切れば並走できる**
  → [[reference_two_sessions_built_the_same_thing]]（同じファイルを2体が触ると巻き戻る）。
- **★層の分け方の型**（今回に限らず使える）

```
層1 検知   起きたことが人に届く          ★最後の砦。これだけでは減らない
層2 予防   そもそも起こせなくする        ★本命。フック・権限・型で不可能にする
層3 解除   起きた後に自力で復帰できる    ★詰まりが残ると層1が鳴り続けて溶ける
```

## ★索引にしか無かった1件を本文へ戻した（2026-09-13）

`MEMORY.md` のこの行が683バイトまで伸び、上限（25KB）を超えた。
**★索引を詰める前に、そこにしか無い記述を本文へ戻す**（[[feedback_memory_index_hygiene]]）。

### 🔴2026-09-07 有璽氏「記録しました を何回も言って同じことを繰り返してる」

**★記録は再発を止めない。** memory へ書いた回数と、同じ失敗を繰り返した回数は独立している。
止めたければ**その作業の入口に検問を置く**しかない。

```
サイトの出し直し   ★手順を1本へ固めた ~/lifestandup-wp/redeploy.sh
                   ★ng_words.txt の語が残っていればデプロイを止める
                   ＝「気をつける」ではなく「通れない」形にした
担当の完了         ★~/.vivid-relay/after_agent.sh が見張り、Slackへ出す
                   ＝終わったことを人が見に行かなくてよい形にした
```

- **★「記録しました」で終わる報告は、次に同じことが起きる合図。**
...
## 🔴2026-09-14 有璽氏の恒久指示 ── 「動くとこまで責任持て。★動き続けること」

> **「作りっぱなしで終わらせんなよ。全然動かんくなってんの。腐るほどこれまでも見てきとんねん。
>   動くとこまでちゃんと責任持ってやれ。★動き続けること。」**

**★実測（2026-09-14・この指摘の直前に数えた）**
```
memory に「機械化した」と書いた回数     ★23回
作ったスクリプトの実在                   ★5本とも在る
★そのうち実際に呼ばれているもの          ★1本だけ（check_path_duplication）
  kb_schema_check          参照0件 ＝ ★作っただけ
  check_index_line_length  参照0件 ＝ ★作っただけ
  hook_output_guard        参照0件 ＝ ★作っただけ
```
★**作る → 登録しない → 動かない。これを4回繰り返していた。**

★**「機械化した」と言ってよい条件は3段。1つでも欠けたら言わない。**
```
① 作った          ファイルが在る
② ★繋いだ        cron / フック / 既存スクリプトから★実際に呼ばれる場所に在る
                  （★「登録は人の手待ち」で置いたものは、ほぼ全部そのまま死ぬ）
③ ★動き続ける    ★発火をログに残し、★毎日その件数を数える検査がある
                  ＝0件の日が続いたら🔴で気づける
```
★②で止めたものを「機械化した」と書かない。**「作ったが繋いでいない」と書く。**
★③が無いものは、静かに止まっても誰も気づかない（今日の4本がまさにそれ）。
→ [[reference_heartbeat_proves_life_not_results]]（心拍は生死しか見ない）
