...
## ★2026-09-07 有璽氏の要望 ── 案件単位の見える化＋キャラのアイコン

> 「**窓口は一つにしてほしい**けど、**1ページで複数またがって見づらいことは望ましくない**ので、
> そこの**見せ方は考えて**ください。
> **それぞれの案件単位で各エージェントがどういう動きをしているのかを可視化**できるように。
> **それが全体としてどういうふうに見えているのかも**教えてください。
> で、そこで**動いてるか動いてないみたいなものもちゃんと考えれるように**してください。
> ちょっと分かりにくいので。
> **キャラのアイコン、それぞれのAIエージェントごとのアイコンとかも作ってわかりやすく**してください。」

```
★満たすこと
  ① 窓口は1つ（URLを増やさない）
  ② ★1ページに詰め込まない（見せ方＝階層／切替を設計する）
  ③ ★案件ごとに、どの担当が何をしているかが分かる
  ④ ★全体像も分かる（案件をまたいだ俯瞰）
  ⑤ ★動いている／動いていない が分かる
  ⑥ ★担当ごとのキャラのアイコン
★前提（9/7 有璽氏）
  「★文字が多いと読む気なくす」 → 文字でなく形・色・アイコンで見せる
```

- ★**新しいページは作らない。**既にある稼働盤（`fukuchi-kadoban`）へ面を足す。
  → [[feedback_check_the_archive_first]]（無いから作る、をしない）
- ★いまの稼働盤が出しているのは**仕組みの稼働**（自動処理58件・担当13体・台帳）。
  **★足りないのは「受けた指示 → いまの状態」＝案件の進み具合。**
- ★担当は13体。アイコンはデザインの仕事 → **フランキー**。
  実装は **ピタゴラス**。★2手に分ける。
