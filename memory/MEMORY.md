...
- [「できない」の前に試す](feedback_verify_before_declining.md) — **★9/9 プラグインを★名前で案内したら同名の別物が入った（Smash Balloon→QuadLayers製が入った）。★同名が多い領域(WPプラグイン/拡張/アプリ)は★一意なURLかIDで渡す＋★入れた後に左メニューへ出る名前まで書く**／**🔴9/9「有璽氏がやるしかない」と言った4件を検証したら★3件は当方で埋まった（2件は読むだけ・1件は過去の記録に在った）。★「あなたがやるしかない」も憶測で言わない＝人の時間を奪う**／憶測で断らない。理由＋やりますかまで／★人へ渡す手順は「挿入位置」でなく置き換え後の全文で(2026-08-23)／**🔴9/7 素のターミナルへ渡すコマンドに`!`を付けない＝zshでは否定演算子。`!`はClaude Codeの入力欄だけ**／**★実測した「できない」も腐る。迂回路を作ったら再測の合図を同じ場所に書く（crontab不能は18日後に直っていた）**
...
- [IGをサイトへ出す](project_lifestandup_website_wordpress.md) — **✅9/9接続完了。残＝①フィード作成→ショートコード→埋め込み ②★余計に入った4プラグイン(WPChat/WPConsent/YouTube/接触)を停止。★真因=STEP2が既定で全部ON。★プラグイン案内は「同時に入るもの」まで数える／★出ない画面を予言して人を待たせない**
