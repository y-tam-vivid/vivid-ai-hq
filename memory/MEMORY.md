...
- [直った基準を目的側に](reference_verify_outcome_not_mechanism.md) — ★9/10 機械0/56でも★要素がセクション外へ消えていた(目視が拾った)。件数0を合格の根拠にしない／9/9はみ出し防止≠矢印どおり
...
- [1経路で断定するな](feedback_one_route_is_not_verification.md) — **🔴9/10「限りなく漏れない形にして」＝★限界の申告で止めるな。できないと書いた瞬間が★塞ぐ工夫を出す場所（名前で出ないなら★前回の構造と比べる）／見る順は①実物②memory③WORKING.md**
...
- [Web制作ルールを資産に](project_web_build_rules_asset.md) — 🔴9/10有璽氏「★オートレイアウトの設計不足で崩れる」＝真因の指摘。★sp/tabletは今も崩れ・FV最優先・★可視化の要望。9/12議論
