...
- [IGをサイトへ出す](project_lifestandup_website_wordpress.md) — **✅9/9フィード作成まで完了。残＝右上「埋め込み」でショートコード取得→サイトへ。★余計に入った4プラグインを停止。★同日4件が同じ根＝★実物を見ずに人の画面を指示した（名前で案内／出ない画面を予言／同時に入るものを数えず／★押す場所を書かず）**
