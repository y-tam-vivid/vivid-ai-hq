...
- [同じ依頼が2つのセッションへ入る](reference_two_sessions_built_the_same_thing.md) — **★9/9 並列の前に数えるものが3つに＝①触るファイル ②★ポート/プロセス(redeploy.shは8750をpkillする＝走行中の担当を殺す) ③外部の枠。★公開スクリプトが何をkillするか実行前に読む**／**🔴9/9 5例目＝★worktreeで分けても同じCSSの同じ箇所で競合（7ファイル）。★投げる前にmergeのドライランまで見る。ファイル名の重なりだけでは分からない。★後始末の判断は担当でなく窓口が出す**／**⛔9/6 巻き戻りの真因＝cron*/15の無条件上書き。🔴9/7・9/8 3・4例目＝agent_watchdog／vivi_patrolが申告なしでcron*/5稼働（心拍なし＝心拍前提の検査は構造的に見逃す・除去の翌日に生える）／🔴9/8 5例目＝★記憶ファイルが二重化（同じ発言から2本）。真因は19時間の枝分かれで記録が相手へ届かないこと。★棚卸し・記録は取り込んでから始める**
