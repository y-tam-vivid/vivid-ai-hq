...
- [一人で抱えるな](feedback_use_the_team_not_alone.md) — **ビビは担当を動かす役。自分で作業する役ではない**。2026-08-20実証＝一人で書いた7本に3体を並列で当てたら24件の不備(単独では1件も見つけられず)。★台帳へ書く前は必ず つる(data-auditor)の検査を通す。足りない役は作ってよい／**★窓口はビビ一人。メインセッションは取次ぎに徹する（2026-08-20）。サブエージェントは有璽氏と直接やり取りできないので「交代」は構造上できない＝自分で判断も作業もせずビビへ渡す。例外はクラウドroutine(RemoteTrigger)だけ＝本体しか操作できない**
