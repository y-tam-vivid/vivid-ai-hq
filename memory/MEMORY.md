...
- [法人番号が埋まらない真因は台帳の社名欄](reference_ledger_name_blocks_corp_match.md) — **151件の空欄のうち書けたのは6件**。国税庁側に無いのではなく①法人名＋施設名を1セルに同居（医療法人幸人会 田島クリニック）②社名の誤記（ジブラルタ**ル**生命＝実在は「ジブラルタ生命」）③2社をスラッシュ連結、で突合が死ぬ。★社名完全一致でも都道府県が食い違えば別法人／**★`if len(候補)>1` を連ねると1件に絞れた瞬間に後段の検査が全部飛ぶ＝解散済み法人番号を確定しかけた**（絞り込みと適格性検査を混ぜない）。**★欠損には「汚れで埋まっていない約30件」と「個人事業主でそもそも存在しない約20件」の2種類がある。分けないと永遠に終わらない**(2026-08-20実測・要判断79件の内訳)。実体=`~/.vivid-relay/corp_number_fill.py`（既定は読むだけを実測済・確定4件/要判断79件が再現）
