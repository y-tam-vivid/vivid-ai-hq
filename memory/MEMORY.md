...
- [特別な理由がなければ全ページ統一](feedback_uniform_unless_reason.md) — ★9/8で5回目。★素材にも。「暗い」でなく「異なる」＝基準値1つで揃える
...
- [業務の単位まで割る](feedback_break_down_to_the_work_level.md) — **🔴9/8「一口にRPOと言っても何をやるのか分からない。具体性がない。中身がなくない？」＝集計で止めるな。★人を割り当てる先は困りごとでなく★業務。1行＝1業務で誰が/いくら/自社可否を揃える。価格帯は参考値でも必ず添える。掛け合わせ候補も出す。★形はスプレッドシート・Webは足すもの／**★価格は①福祉の同業他社→②他業種→③空欄＋未確定の順で探す。下限/平均/上限の3つ・出典つき。1社なら「相場」と呼ばない／★洗い出しは前後へ伸ばす(RPOなら母集団形成〜面接練習〜定着の研修・面談まで)**
...
- [1経路で断定するな](feedback_one_route_is_not_verification.md) — 数・存在・状態は2経路／0件は別法で数え直す／列挙は絞らず全部出す／**★限界の申告は免罪符でない／有璽氏が数字に違和感を示したら数え直す（逆に「合っている」と言われたら測らない）／★9/7 ログを途中まで読んで「無い」と言わない・レジスタを見ずに「心拍が無い」と誤断定した／**★9/8「0件」の正体が自分の数え方だった＝TSV指定で返りがMarkdown表。★0を報告する前にファイルのサイズと更新時刻を見る（23KBのファイルが0件はあり得ない）。pgrepでの生存確認も起動直後に空振りする**
...
- [日本語に別の文字が混入する](reference_unicode_escape_kanji_swap.md) — ★\uエスケープで漢字化け／**★9/5機械で解消（Stopフック検査4）。9/9に本番3例目「этот」を★機械が先に検出し訂正できた。意味の同じ外国語に化けるので目視では素通りする**
