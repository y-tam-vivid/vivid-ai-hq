...
- [検出でなく不可能にする](reference_make_it_impossible_not_detectable.md) — ★1975年に「検出型は原理的に不完全」と結論済。規範配下をread-onlyに
...
- [心拍は生死しか見ない](reference_heartbeat_proves_life_not_results.md) — ★成果の数字と期待値を持つ。基準は測る前に決める
- [誰も拾わない警告は無に等しい](reference_a_warning_nobody_owns.md) — ★数えて表示して警告にしない型。同じ指摘の連続日数を数える
...
- [権限も環境の一部](reference_permissions_are_part_of_the_environment.md) — ★allowに*があってもaskが勝つ。予定作成は意図的にask／代替は.ics／**★担当も書けないことがある(allowにEditがあってもdontAskで拒否)。規範の制限と権限の制限が重なると誰も触れない領域ができる→resumeでなく新規起動で試す**
