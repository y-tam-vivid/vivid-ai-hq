...
- [営業ワークブックは触る前に戻せる状態を作る](feedback_sales_workbook_hands_off.md) — 禁止を撤回(2026-08-13本人)。**変更は①バックアップ→②diff→③承認→④実行**。バックアップは99_バックアップへ`_バックアップ_YYYYMMDD-HHMM`で1作業1本・原本はIDで見分ける。buildWorkbook・applySchemaV3のみ実行禁止のまま。**自動バックアップは停止中(8/10分が無い・実測)**
