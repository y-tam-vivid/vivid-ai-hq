...
- [営業×議事録の統合設計](project_sales_minutes_integration.md) — **2026-08-22 有璽氏が全論点へ回答して決まった骨子**。名刺も受付フォームと同じ動線(照合→人が○×→台帳)／議事録の昇格は有璽氏=手動チェック・社員=AIが重複と機微を見て自動格納＋タスク振分／**★逆向き(営業→過去の会話)を辿れるよう設計する＝全社へ昇格したものだけ見せる**／対象外の判別は人が確認し根拠を残す／**★kintoneの結合キーは社内顧客IDへ寄せる。法人番号は登録時に自動収集**／案件化ルールを設ける／鈴木・柴田は営業ではない
