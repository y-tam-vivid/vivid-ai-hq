...
- [AI活用→発信ネタ化](project_ai_usage_to_content_pipeline.md) — 区切りごとに⑥へ記録＝発信の種。正本は発信ネタDB1つ／迷ったら拾う
- [claude.aiログ→Notion移行](project_claude_ai_logs_to_notion_migration.md) — 約15Projectを📚ナレッジDBへ。空ページ作成済・流し込み待ち
- [AI活用ログDB統合(案A)](project_ai_log_db_consolidation.md) — 単一台帳化。Phase1完了／Phase2(発信ネタDB統合・投稿生成)は保留
- [SNS生成スキルの在り処](reference_sns_skills_location.md) — vivid-sns-*はclaude.ai Project内。★Manusと二重＝どちらが正かは未決
- [Manus AIへの外注](project_manus_outsourcing.md) — 社外の手足。個人IGは投稿実行まで委任／顧客名を渡さない
- [「誤記」と決めつけない](feedback_dont_call_it_a_typo.md) — 実測は過去の写し。「どちらが正か」と並べて聞く
- [119番の投稿標準型](project_f119_post_standard.md) — ★Manus停止中(〜8/25 08:00)。差し戻し送信済で復旧待ち／見本はI-03/01とI-04/05
- [テイストは素材ではない](feedback_taste_is_not_a_sticker.md) — 画風の指示に画像を渡すと貼り込まれる。表紙案の採用時は2枚目以降も同時に決める
...
- [機微の二層管理](feedback_confidential_two_layer_rule.md) — 原則共有・機微だけ`_機微`。★AIが「機微そう」で隔離を確定しない
- [Artifactは積む](feedback_artifact_accumulate_dont_replace.md) — 作り替えず同じ1ページへ積む。★縮小版は「縮小版だ」と必ず言う
...
- [営業案件管理：現在地](project_sales_workbook_read_first.md) — 起点は🧭全体像1枚。★機械が新規と判定して台帳へ書かない
...
- [セッションからcrontabは書けない](reference_cron_write_blocked_in_session.md) — bin/cron/へ投函→cron_apply.shが入れる。投函≠登録・最大15分後
...
- [自動処理レジスタ(心拍)](project_automation_register.md) — ★心拍名は両機で共有＝後勝ちで化ける／実体は3経路
...
- [フォルダ分類は誤答を強制する](reference_folder_classification_forces_wrong_answers.md) — 1つしか選べない置き場に判断を置かない
...
- [検査が偽陰性を出す](reference_reference_audit_false_negative.md) — 「検査した」と「正しく検査できた」は別。定数経由の参照を落とす
