...
- [営業×議事録の統合設計](project_sales_minutes_integration.md) — **2026-08-22 有璽氏が全論点へ回答して決まった骨子**。名刺も受付フォームと同じ動線(照合→人が○×→台帳)／議事録の昇格は有璽氏=手動チェック・社員=AIが重複と機微を見て自動格納＋タスク振分／**★逆向き(営業→過去の会話)を辿れるよう設計する＝全社へ昇格したものだけ見せる**／対象外の判別は人が確認し根拠を残す／**★kintoneの結合キーは社内顧客IDへ寄せる。法人番号は登録時に自動収集**／案件化ルールを設ける／鈴木・柴田は営業ではない／**★2026-08-22実測＝promote_minutes.pyは既定dry-run・cronにもdaily_jobsにも登録が無く、有璽氏がチェックを入れても昇格しない。「チェックは人・昇格は機械」へ寄せる（載せる前に対象件数を数える）**／**★架電のフローは定まっていない（06_テレアポリストは実データ0件・40のTEL種別は27件中0件＝箱はあるが誰も使っていない）**／**★「対象外」はコード内蔵の固定リストでAIが自動判定し、どれを外したかは残らない。Slack通知の口は既存2本(隔離・重複)を流用すれば作れる**／**★2026-08-22決定＝架電フローは使う(ただしリストを増やさない。05はSalesBreakerのミラーで毎朝上書き・06は実データ0件)／対象外をSlackへ通知する。★「対象外の通知」だけでなく、機械が誰にも聞かず決めているものを全部洗い出して1件1通知にまとめる**
