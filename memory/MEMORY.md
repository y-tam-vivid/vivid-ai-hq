...
- [確認は溜めて報告は溜めない](feedback_batch_the_checks.md) — 承認6項目だけ／両機に入れる／★溜めるのは確認・報告は進んだ時点で出す／9/7「随時共有」＝毎回の冒頭に走行中の状態を置く／**🔴9/8 窓口は「10分後に出す」を約束してはいけない＝話しかけられるまで1行も実行しない。定期報告はminiのcronからSlackへ。★シェルが返すPIDは起動の証拠にならない＝ログか成果物の実在で判定**
