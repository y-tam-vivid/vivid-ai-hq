...
- [制作は原則miniへ](reference_offload_long_work_to_mini.md) — ★9/5恒久化。MacBookで担当を起こさない／**🔴9/5「miniが止まらないように」が要件化。止まる型3つ(承認ダイアログ/30分打切り/判断待ちの塞ぎ)。⛔AskUserQuestionが原因は誤診＝非対話にそのツールは無い(実測)。判定は CLAUDE_CODE_ENTRYPOINT。★止まったかはログでなく transcript の mtime で見る／**★9/6 投げた直後に無言で死ぬことがある(同じ指示文で2回死に3回目に生存＝非決定的)。ログには起動時の2行しか残らない。必ず20〜30秒後にpgrepで生存を確認する。★機械化＝`run_agent.sh`（投げ直し＋失敗時のみSlack）／**⛔9/7 真因判明＝miniのOAuth期限切れ。投げ直しでは直らない・人のログインが要る。`claude --version`が返ることを動く根拠にしない。ログを途中まで読んで「何も無い」と言わない**
