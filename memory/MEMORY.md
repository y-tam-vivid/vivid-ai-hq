...
- [実物を読む](feedback_read_the_artifact_not_the_copy.md) — ★欄名が無いは入口(隣列を見る)／★渡すのは行でなくセル／**★有璽氏が言うのは症状。原因は別(「左に余白」の真因は番号バッジが画面外へ飛んでいた)。直す前に実物で1回見る**
...
- [担当を長く使うと落ちる](feedback_use_the_team_not_alone.md) — ★9/4実測。600秒無応答の5回中4回が1日resumeし続けた1体（累計300万トークン超）。**新規起動の6体は全員完走。工程ごとに起こし直す・区切りでcommit**
...
- [日本語に別の文字が混入する](reference_unicode_escape_kanji_swap.md) — ★\uエスケープで漢字がすり替わる／**★9/4 エスケープ無しでもロシア語が1語混入（「特に」→「особенно」）。エラーにならない＝長文は送る前に自分で1回読む**
...
- [長い処理はminiへ切り離す](reference_offload_long_work_to_mini.md) — ★スリープで全体停止(5体)。ssh+nohupで投げる／セッションはこのまま
