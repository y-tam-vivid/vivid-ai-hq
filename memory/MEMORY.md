...
- [判断はSlackのボタンで返す](project_ask_hub_push_decisions.md) — 🔴9/5 4回目＝1通でなく**全経路を数えて揃えろ**。★真因は拾う人不在でなく「押せないものが混ざっている」＝クリックだけなら有璽氏は1〜3分で答える／DMの判断は全部ボタンにした／★押されても聞いた側へ返らない＝毎回answer_of()／★kindは8種のみ・仕込む前にpreview／★承認ダイアログはSlackで解けない(9/5の「走行中」宣言は実物0＝sha256一致・9/6着手)／★催促は新しく聞き直さない＝同じ受付番号で再投稿しts更新（9/5 #8330f0で実施）／**🔴9/6 AIが自分の問いに自分で答えて畳んだ(#960e67・指示はビビ)。台帳はanswered=人/機械を区別しない。byに指示の在処を書けないなら畳むな。一度出したものを畳むのは不可逆側**／**★9/6 同時に4件出したら「はい」が紐づかない＝問いは1件ずつ出す。一語返信を推測で当てない**／**★9/6 実装した→[[reference_permission_request_hook_headless]]。効くのは「対話かつdontAsk以外」の1通りだけ・非対話は発火0（10ケース）・🔴async:trueだと決定が捨てられる＝settings.jsonからasyncを消しtimeout600へ(人の手・両機)が残。★通知83件中63件が`hook_selfcheck.py`のニセ(08:20/08:40に各18件・18日間)→session_id無しは投稿しない形にした。★件数は全履歴403件を数えた値（別セッションの「24件中17件」は窓が短い）**／残＝tell()の自由出力2本と#09の2本
