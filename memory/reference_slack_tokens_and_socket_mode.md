...
★続報   08-28 朝ロビン再測 ── **8/27 も 13:17 / 16:08 / 18:30 の3回。累計17回。**
        ＝1日たっても発信元は追われておらず、押した人へエラーが返り続けている。
        **暫定の手当て（形式外の value には返信せず、ログだけ残す）も入っていない。**
        次に追う人が1回で分かる足がかり ── payload の `api_app_id` /
        `message.bot_id` / `channel` をログへ出す（`handle_block_actions`）
★続報   08-29 朝ロビン再測 ── **8/28 は7回（12:03/13:12×2/14:54/14:55/21:52/23:44）。
        累計24回。日別 8/25=1・8/26=13・8/27=3・8/28=7。**
        ★**4日連続で「数えて記録しただけ」。** 発信元の特定も、payload のログ出しも、
        暫定の手当ても、1つも入っていない。毎朝の棚卸しが**対処の代わりに**なっている
        → [[reference_a_warning_nobody_owns]]。
        ★次にこれを読んだ人は、数え直す前に**足がかり3つ（api_app_id / bot_id / channel）を
        ログへ出す1行**を入れること。それが無い限り翌朝も同じ更新が続く
...
## ★UUID value ── 9日目。8/29 に書いた「次に読む人へ」の指示は実行されなかった（2026-09-02 ロビン実測）

上の節で 08-29 に **「次にこれを読んだ人は、数え直す前に足がかり3つ
（api_app_id / bot_id / channel）をログへ出す1行を入れること」** と書いた。

```
経路1  slack_socket.py を grep       api_app_id ＝ 0件
                                     bot_id ＝ 2件（どちらも別用途・:500 :507）
                                     → ★足がかりの1行は入っていない
経路2  ログの日別件数を数え直す       8/25=1 8/26=13 8/27=3 8/28=7
                                     8/29=0 8/30=0 8/31=1 9/1=6  累計31
                                     （8/29朝の申告「累計24」＝1+13+3+7 と一致 ✓）
```

**予告どおりになった。** 「それが無い限り翌朝も同じ更新が続く」と書いた本人が、
4日後に同じ更新を書きに来た。**8/29・8/30 が0件だったのは直ったからではなく、
その2日は誰も押さなかっただけ**（9/1 に6回で再燃）。
★0が2日続いても解消の証拠にしない → [[feedback_one_route_is_not_verification]]。

- **★memory に「次に読んだ人が」と書いた指示は、誰にも割り当たっていない＝実行されない。**
  毎朝ここを読むのは棚卸し役（ロビン）で、**コードを触らない役**。
  構造的に、記録役へ対処を書けば「数え直して追記する」に化ける。
  → 詳細な型は [[reference_a_warning_nobody_owns]] の該当節へ。
- **★今回は担当を名指しする。** 対処＝`handle_block_actions`（`slack_socket.py:518` 付近）で
  `payload` の `api_app_id` / `message.bot_id` / `channel` をログへ1行出す。
  **担当＝ピタゴラス**（コード変更役）／検査＝ステラ。`~/.vivid-relay/` は git 外なので
  両機へ手で配る（[[reference_fix_where_git_reaches]]）。
- 押した人には今も `fail_reply(…, 'ボタンの中身が読めませんでした')` が返り続けている
  （[[reference_silent_failure_kills_adoption]]）。**発信元が分かるまで受け口を広げない**方針は維持。

## ★disconnect のあと「1秒後に再接続」が9回並んだ ── バックオフが効いていない（2026-09-02 実測）

上の「disconnect は異常ではない」は**いまも正しい**。だが 09-02 06:29 に
**19秒で9回**の接続／切断が起き、うち8回は `too_many_websockets` だった。

**原因はSlackではなくこちら。** `backoff = 1` のリセット位置が「ソケットが開いた時点」で、
1秒で張り直すため Slack から見て古いソケットがまだ生きている ＝
**自分の速すぎる再接続が自分で拒否を作っていた。**
機構と直し方は [[reference_retry_backoff_resets_too_early]]（担当＝ピタゴラス）。

**★見るのは disconnect の件数ではなく「秒数が増えているか」。** 同じ数字が並べば黒。
