...
## ⛔2026-09-09 22:32 訂正 ── 更新間隔は★20分おきではなく★30分おき

```
記録にあった  「Blobのpushed_atが12:40:11→13:03:06と★20分おきに進む」（9/8）
実測(crontab) ★`3,33 * * * *` ＝ ★毎時03分と33分＝★30分おき
              22:03更新 → 22:32時点で未更新は★正常（次は22:33）
★原因        当時の実測1点（12:40→13:03＝23分）を「20分おき」と★概算で書き、
              ★cron の定義そのものを見ていなかった
★型          ★間隔は「観測した差」ではなく★定義（crontab）で書く。
              観測は1点だと必ずずれる＝★「遅れている」の誤検知になる
```
