...
## ★2026-09-15 事故は起きなかったが、判定の材料が無かった

**★並列で2体（リリス／ピタゴラス）を走らせたときの実測。**

```
リリスの出口   「★ポート8799で別プロセスのhttp.serverが稼働中を検知したが、
                当方が起動したものではない＝他セッションの作業中と判断し触っていない」
★実際の正体   ★同時に走らせていたピタゴラス自身の検証サーバ（09:50:49 起動）
結果          ★触らなかったので実害0。★ただし正しく判定できたわけではない
```

**★「自分のではない」までは分かる。「誰のか」は分からない。**
リリスが慎重だったから止めなかっただけで、**「残骸だろう」と判断していたら
走行中の兄弟を殺していた。**

**★対策＝投げる側がポートを割り当てる。**

```
指示文へ1行    「★あなたが使う検証ポートは 87xx です。他のポートには触らないでください」
              ★8750 は本番同等の検証・8799 以降を担当ごとに割り当てる
これで        ★知らないポート＝確実に他人のもの、と★根拠を持って判定できる
```

**★いま並列で何体走っているかを、指示文に書いて渡す。**
「もう一方が何を触るか」は既に書いている（上の節）。**★ポートも同じ扱いにする。**

→ [[reference_two_sessions_built_the_same_thing]]／[[feedback_one_session_split_by_owner]]
