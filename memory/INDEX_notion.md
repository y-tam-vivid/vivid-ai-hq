...
- [Notion MCPの読み取り制約](reference_notion_mcp_read_limits.md) — ★404の切り分けは/v1/search／DBテンプレはAPI経由で効かない／実データはview mode／**★書く側も壊れる。プロパティ値がMarkdown解釈され`*/5`と`foo_bar.py`が化ける＝REST API直で書き、fetchで読み返す**
