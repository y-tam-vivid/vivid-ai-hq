...
- [Fable Style改善ループ](project_fable_style_improvement_loop.md) — 週次で自己改善／★種の入口が汚い＝corrections.logに有璽氏の発言でない行が混じる（累計38%＝起動プロンプト＋自己点検のテスト文字列）。**8/30は12件中7件が探針由来で悪化**／★8/30に hook_selfcheck.py を直す機会があったのに隣の汚染を塞がず。直し方はHOME差し替えの写し・2行（ステラ/ピタゴラス領域・依頼未提出）／**★9/19 30日9回書いて発注0回だった→★この日初めて system-developer へ発注（受け入れ基準＝翌朝の材料にこの2件が出ないこと）。★測り直したら主因は探針16%でなくAI自身の起動プロンプト44%＝ロビン/つるの起動文が翌朝「有璽氏の発言」として返る自己参照ループ。直近7日のうち5日は有璽氏の実発言0件**／★対症「秒までゼロの行を先に落とす」は探針しか落とせない（起動プロンプトは秒が0でない）
...
- [JFBI調査研究サイト](project_jfbi_research_site.md) — ★9/24 協力団体a’s(岡崎)の詳細ページへ写真10枚を掲載・並びをメンバーと同じ形へ・掲載文2か所を修正。★2団体目は未受領。正本は~/jfbi-research-site/README.md
