...
**広報PR・SNS・Manus・デザイン・成果物の見せ方**

> この分野の作業に着手したら読む。正本は各ファイルの本文。ここは索引。
> **上限は無い。** 毎ターン届く `MEMORY.md` と違い、必要なときだけ読まれる。
> 呼び出しの対応は [[INDEX_担当別]] にある。

- [Fable Style改善ループ](project_fable_style_improvement_loop.md) — 週次で自己改善／★種の入口が汚い＝corrections.logに有璽氏の発言でない行が混じる（累計38%＝起動プロンプト＋自己点検のテスト文字列）。**8/30は12件中7件が探針由来で悪化**／★8/30に hook_selfcheck.py を直す機会があったのに隣の汚染を塞がず。直し方はHOME差し替えの写し・2行（ステラ/ピタゴラス領域・依頼未提出）／**★9/10も未修正＝11日継続。08:20:00と08:40:00ちょうどの31件は全部探針。毎朝の棚卸しは秒までゼロの行を先に落としてから数える**
...
- [有璽個人IGの運用](project_yuji_personal_ig_operation.md) — ★週次の定例／画像の在庫3系統／本文200〜215字／**Notionの投稿アーカイブに素材画像列＋「未投稿の在庫」ギャラリーを新設(日付は入れず画像で選ぶ)**／9/11でカルーセル在庫が尽きる
...
- [オレンジワークス ポートフォリオサイト](project_orangeworks_portfolio_site.md) — **★9/14 実績42件+2ページ(private)。住所は全34ページ「ハイツ101」へ統一済(公開面で旧表記0)。表記はオレンジワークス藤井寺／法人リアンライフ株式会社(privacy修正済)。🔴回答待ち#1e8e0b一覧・#6eab01略称・#65db27求人雇用主**／Desktop側SEOと並走(鍵claude-kiji・触らない)
...
- [かわちばなし・絵は都度作る](project_kawachibanashi_portal.md) — ★9/13訂正 先回りで揃えない。実イベントを埋める過程で1枚ずつ作る
...
- [揃うまで待たない](feedback_dont_wait_for_the_full_set.md) — ★9/12 **穴が埋まるときの答えは3種類**＝①値が決まった→埋める ②★「無い」と決まった→**消す**（未確定と同じ箱に入れない・残る文が成立するか実物で読む） ③まだ決まらない→公開面から外しコメントと引き継ぎ書へ退避。／★9/8 複数件は1件ごとに独立差し替え可能な構造に
