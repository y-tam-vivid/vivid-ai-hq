...
## ★★同じフォルダの中で NFC と NFD が混在する（2026-08-29 ピタゴラス実測）

いちばん厄介なのはここ。**`/System/Library/Fonts` の中で形式が揃っていない。**

```
NFC で保存        ヒラギノ角ゴシック W3.ttc          ← これ1本だけ
NFD で保存        W0 / W1 / W2 / W4〜W9 / 丸ゴ / 明朝  ← 残り全部
```

- **★「1本で試したら通った」は、他も通る根拠にならない。** たまたま NFC の W3 で試すと動き、
  W6・W9 を使う本番で落ちる。**混在は、テストをすり抜ける形の壊れ方をする。**
- **★だからパスをハードコードしない。** `glob` で実ファイル名を取得し、
  **NFC正規化して照合し、開くときは取得した実パスをそのまま渡す**（OS側が正しく解決する）。
- 同じ型は Drive・Notion・スプレッドシートの日本語名にも起こりうる。
  **一覧から取った実名を使い、自分で組み立てた文字列で照合しない。**
