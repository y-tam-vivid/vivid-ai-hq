...
## 🔴 MCPに「行を削除するツール」が無い（2026-09-09 網羅調査で確認）

```
在る    fetch ／ create-pages ／ update-page ／ move-pages ／ duplicate-page
無い    ★1行（1ページ）を削除・ゴミ箱へ移す手段
        `update-data-source` の `in_trash` は★データソース全体の削除設定。行単位には使えない
```

- ★既知だった（[[project_giji_automation_gas]]「1ページ単位のtrash APIが無く最終削除は手動」）が、
  **道具の記録にしか書いていなかったので、担当が案件ごとに探し直した。**
  → **★道具の制約は、案件のファイルではなくここに集める。**
- **★「消せない」を人の手作業に回さない。**多くの場合、消す必要が無い。
  ```
  ◎ ★上書きする（同じキーの行を更新する）
  ◎ ★消さずに状態で表す（実績0＋備考に「旧版」）＝ fukuchi-core
  ✕ 人にNotionを開かせてゴミ箱へ移させる  ★運用が人へ戻る
  ```
- ★どうしても消すなら Notion 公式APIを直に叩く経路が要る（`config.env` の `NOTION_TOKEN`）。
  **ただし本人限定ページはその統合と共有されていないことがある**（2026-09-08、財務室で404）。

## ★select のリネームは「削除して作り直し」（2026-09-03 実測・2026-09-09 実害を確認）

```
名前を足す    ★安全。名前が一致すれば既存の内部IDは保持される
名前を変える  ★内部IDが変わる ＝ 実質は削除＋新規作成
             → ★その選択肢を参照している行が静かに外れる
```

**★2026-09-09、27行が入った状態で5件の科目を改名する必要が生じた。**
参照していた5行を先に特定し、**その列だけ null にしてから改名した**（バックアップ済み・可逆）。
**★「中身が空のうちに名前を決める」が理想。入ってしまったら、参照を外してから変える。**
## ★財務室（本人限定ページ）は、mini側の内部インテグレーションから読めない（2026-09-08 実測・ナミ）

**「エージェント経由でBashを叩くセッション」には Notion MCP コネクタが接続されていない。**
`ToolSearch` で "notion" を探しても該当0件。使えるのは `~/.vivid-relay/config.env` の
`NOTION_TOKEN`（内部インテグレーション「Chatworkリレー」）1本だけで、これは
**財務室配下（本人限定）の予実管理DBとは共有されていない**。

```
実測1  このセッション自身が GET /v1/databases/{id} を直叩き → 404 object_not_found
       （「関連するページやデータベースがインテグレーション『Chatworkリレー』と
         共有されていません」と明記）
実測2  mini上で新規に claude -p を起動し同じDBを取得させても → 同じ404
       （新規セッションにもNotion MCP接続は無い。同じトークンで同じ結果）
```

- **★これは「Notion側で404」の話ではなく「このマシン・この起動経路にはそもそも
  Notion MCPが繋がっていない」という前提の話。** 過去の記録（本ファイル上部）にある
  `notion-fetch` / `notion-query-data-sources` 等のツールは、**Notion connector が
  接続された別の面（MacBook側のセッション種別・claude.ai/code等）でのみ使えていた**。
  mini上で `claude -p` や Bash 経由の Python から Notion を読み書きしたい場合は、
  内部インテグレーションのトークン（config.env）を使うほかなく、**そのトークンと
  対象DBが共有されているかを先に確かめる**（`POST /v1/search` で接続済みDB一覧に
  出るか、が確実な切り分け方 → 本ファイル23-34行と同じ手順）。
- **★機微性の高いページ（本人限定）ほど、この制約に当たりやすい。** 共有範囲を
  広げる話になるので、**内部インテグレーションへの新規共有は勝手に進めない**
  （fukuchi-core「正本の削除」等と同格の重い操作ではないが、機微データの露出範囲を
  変える操作＝要相談）。
- 関連 [[project_cfo_agent]]（勘定科目 正式名称の突合作業で発覚）。
