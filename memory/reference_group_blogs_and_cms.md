...
足した行   ブログ5本（NPO・ILIFE・ふくち。・ビビッド・オレンジワークス）
           ＋ NPO Instagram（★2026-09-03 訂正 ── **未作成**。有璽氏に確認済み。
             ★旧記述「開設済みだが未運用・アカウントIDが未取得」は誤り。
               「IDが取れていない」を「開設はされている」と書いていた）
...
## ⛔訂正（2026-09-05）── NPOにブログのページは無い。403の読み違いだった

**「/blog が403＝入口のパスが別」は誤り。curl では何を叩いても403だった。**

```
真因      ★Cloudflare のボット判定。★トップページ自体も 403 を返していた
          ＝ 「403だからそのパスが無い」ではなく「この取得方法では何も見られない」
通る経路   ブラウザ（Claude in Chrome）。同じURLが普通に開く
```

**実測（2026-09-05・ブラウザで2経路）**

```
経路1  トップのナビゲーションを全件読み出し
       ホーム／サービス内容／スタッフ紹介(準備中)／お問い合わせ／会社概要／概要／
       プライバシーポリシー ＝ 7つ。★ブログ無し
経路2  sitemap.xml → 5ページのみ。★ブログ無し
→ 一致。★ページ自体が作られていない（有璽氏の「各サイトにブログはあります」は
   NPOには当たらなかった）
```

- **★curl が403を返したら、まずトップを叩いて切り分ける。**
  トップも403なら、サイトの構造ではなく取得方法の問題。
  **1本のパスだけで「無い」と決めない** → [[feedback_one_route_is_not_verification]]
- **★Jimdo/Wix等のホスティングはボット遮断が既定で入っていることがある。**
  実物を見る必要があるならブラウザを使う。
- NPOへ載せるには ①Jimdoでブログ機能を有効化 ②「お知らせ」の固定ページを作る
  ③今回は見送る、のいずれか。**有璽氏の判断待ち（2026-09-05）**
