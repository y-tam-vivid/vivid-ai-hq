...
## ✅2026-09-15 決着 ── ★3択のどれでもない。「NPOサイトを作り直す」

**上の①②③は全部外れだった。有璽氏の答えは④＝サイトごと新しく作る。**

> 「今ライフスタンドアップのホームページを作っていて、**ほぼ仕上がりつつある**ので、
>  **その後NPO法人側のホームページも作れたら**と思っています。
>  そこに、**ブログ等の機能も当然実装させていく**ような形かなと思っていました。
>  **ブログとインスタとか、そのあたり全部整えていかないといけない。
>  要するに広報系を全部整えないといけない。**そちらのNPOに関してもね」

```
★順序    ① LIFE STAND UP のサイト（★ほぼ完成）
          ② ★NPO法人のサイトを新規制作      ← ここが次の制作案件
          ③ ブログ機能を実装（Jimdoの有効化ではない・新サイトの機能として）
          ④ Instagram を含む★広報系を全部整える
★対象    ★NPO（南河内こどもステーション）の広報一式
★状態    ★まだ着手していない。LSUの完成待ち
```

**★Jimdoをどうするかは未確認**（残すのか・畳むのか・移行するのか）。
上の「①Jimdoでブログ機能を有効化」は**もう検討しなくてよい**。

### ★これは「子ども広報部」の前提条件でもある

[[project_kawachibanashi_portal]] の子ども広報部は、
**子どもが書いたものをNPOの名前で外へ出す**構想。だが現時点で出し先は
PR TIMES と かわちばなし の2つしかなく、**NPO自身のサイトに置く場所が無い。**

```
いま      子どもが書く → ★NPOサイトに置けない → PR TIMES／かわちばなしだけ
これから  子どもが書く → ★NPOサイトのブログ → そこからPR TIMES・かわちばなしへ
```

**★器（NPOサイト）と中身（子ども広報部）は別々に進められる。**
どちらかを待つ必要はない。ただし**同時に揃ったときに効き方が変わる**ので、
子ども広報部の話をNPOの大人へするときは「置き場も作ります」まで言える状態が強い。
