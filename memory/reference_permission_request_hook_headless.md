...
## ★あとから足した分岐が、検問だけを素通りしていた（2026-09-06 commit `8247ea4`）

**同じ日に入れたばかりの「全部見せられないものは遠隔で許可させない」という線引きが、
`mcp__` の呼び出しに対してだけ効いていなかった。**

```
brief() の分岐         長さチェックに到達するか
  Bash / Write / Edit   ◯ 300字・600字・900字の判定を通る
  mcp__◯◯              ✗ ★cut を空にして早期 return していた ＝ 判定へ行かない
```

**実害** ── `notion-update-page` のような**外部サービスへ書きに行く**呼び出しが、
「Notion の notion-update-page」という**1行だけ**で Slack に出て、
**本文を1文字も見せないまま「許可する」ボタンが付いていた。**
危ない順で言えば MCP は Bash より外向きなのに、検問が最も薄い分岐になっていた。

**直し方** ── `_kv_preview()` を新設し、`mcp__` とその他の分岐が**同じ長さチェックを共有する**形へ。
実測16/16合格（短いMCP→ボタンが出て引数が見える／長いMCP→ボタンを出さない を追加。既存13ケースは無傷）。

- **★分岐ごとに検問を書き足さない。判定は1か所に置いて、分岐から呼ぶ。**
  分岐ごとに書くと、**後から増えた分岐だけが検問の外に出る**。増えた本人は
  「既に検問はある」と思っているので、抜けたことに気づかない。
- **★早期 return は検問を飛ばす。** 「この型は表示が単純だから先に返す」という理由の
  early return が、後段の安全判定ごと飛ばしていた。**返す前に、後段に何があるか数える。**
- **★新しい分岐を足したら、既存の検査ケースを流すだけでなく「その分岐用のケースを足す」。**
  既存13ケースは全部通っていた。**通っていたから気づけなかった。**
- ★この型は [[reference_a_warning_nobody_owns]] の「画面に出す項目と警告に入れる項目を
  別々に書かない」と同根 ── **出す経路と判定する経路が分かれると、片方だけが育つ。**
