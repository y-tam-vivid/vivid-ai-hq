...
## ★2026-09-09 有璽氏が本番WordPressの許可を出した ── ★恒久制約の変更

> 「**本番のワードプレスを触っていただいて問題ないです。**」
> （同じ発言で）「**この本番のワードプレスという表現は、今実際に動いているワードプレスに
> 直接触るという意味合いなのか、そうではなくて、それとまた別の未公開のページを
> ワードプレス内で作り、作成するの意図なのか、どちらでしょうか。**」

**★これまでの恒久制約**：「本番サーバー・WP管理画面・DNSへは触らない」
**★2026-09-09 に有璽氏が解除した。★ただし当方から方式Bを提案し、実行はそちらで進める。**

```
A いま動いているサイトを直接触る      ★当方が「本番」と言っていたのはこれ
                                      有効化した瞬間に訪問者にも見える
B ★入れるが有効化しない（★採用）      インストールまで＝★サイトは1ピクセルも変わらない
                                      ★ライブプレビューで自分にだけ見える
                                      ＝ 有璽氏が言った「未公開のページを作る」に最も近い
C サブドメイン等に別で作る            分離できるがサーバー設定・DNSが要る＝手間が増える
```
**★不可逆に近いのは「有効化」の1手だけ。そこは有璽氏に押していただく形にできる。**
**★入れるところまではAIがやれる。**

**★守る順序（許可が出ても崩さない）**
```
① デモ（仮公開）を完成させる → ② その版でzipを作る → ③ 本番へ入れる（★有効化しない）
★「触ってよい」と言われても、★未完成の版を入れない。順序の理由は1行で言う
```
★恒久制約が解けたときは、**★何が可逆で何が不可逆かを1つずつ数え直す。**
「触ってよい」＝「全部やってよい」ではない。

**✅2026-09-09 有璽氏が★Bで確定させた**
> 「ワードプレスへの実装は**Bの推奨のモデルで進めてください**。」
＝ **★本番へは「インストールまで」。有効化はしない。**サイトは1ピクセルも変わらない。
★有効化の1手は、デモ完成後に有璽氏が押す。
