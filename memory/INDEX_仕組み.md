...
- [corrections.logは86%が非有璽氏だった](reference_corrections_log_polluted_by_non_utterances.md) — ★9/19 4経路(探針/AI起動プロンプトの自己参照/task-notification/run_agent.sh定型)を除外。判定は「有璽氏が絶対に書かない形」に限定／**🔴9/20 まだ「直った」と言えない＝修正後に生成された行が0件(2経路)。★受け入れ基準「翌朝の材料から消えていれば完了」は追記型の器では永久に満たされない（過去行は消えない・修正日の分は必ず混ざる）。判定は消えたかでなく★新しく生まれる行で。次の一手＝9/19日中の実発言の有無を別経路で数え、あれば誤除外を疑う**
