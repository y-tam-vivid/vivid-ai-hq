...
- [corrections.logは86%が非有璽氏だった](reference_corrections_log_polluted_by_non_utterances.md) — ✅9/21 偽物は止まった(探針は別ファイルへ逃げ・起動プロンプトは3条件ANDで除外)。★受け入れ基準は「消える」でなく★「新しく生まれない」／**🔴穴は別に2つ＝①PATTERNSが「〜してください」型を1つも拾わない(実測・命令形が網に無い) ②corrections.logは機械ローカルで有璽氏はMacBookで話す＝miniの棚卸しに本物は構造的に来ない。両方★ピタゴラス案件・未着手。★網は件数でなく「本来入る実物」を当てて評価する**
