...
## ★裏側の義務 ── 作った側は、作業の最後に必ず報告を書く（2026-09-06 ビビの指示）

**この手順は「申告が失われたときに検査が止まらない」ための保険であって、申告を省いてよい根拠ではない。**
2026-09-05 は申告が無かったせいで、検査役が旧リビジョンを .xlsx で書き出し全セルを突き合わせた
── **コストは全部、検査する側が払った。**

- **★`claude -p` をバックグラウンドへ投げると、報告はログにしか残らない。**
  ログが警告だけで終わっていても、投げた側からは「動いた」ように見える。
  **走らせた側が完了を判定するのではなく、ログに本文があるかで判定する。**
- **★書く場所は毎回同じにする**（`~/.vivid-relay/<案件名>N.log`）。次に検査する人が探さずに済む。
- 中身は最低4つ ── ①何を書いたか（範囲まで）②何を書いていないか ③読み返しの実測
  ④**申告と実測が食い違った点**。→ [[feedback_never_write_an_unmeasured_number]]
