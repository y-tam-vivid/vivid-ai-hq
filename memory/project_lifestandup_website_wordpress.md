...
```
① トップに②採用の導線を足すか（いま本文ゼロ）
② ★CSSの食い違い148件は意図か時期差か → 工数が12〜20h動く
③ body の style="font-family:Hiragino Kaku Gothic ProN" を残すか
   ★Windowsに同名フォントが無く本文が環境依存になる。外すとNoto Sans JPに揃う
④ .bridge-variant-tag（「B. 虹で繋ぐ」の検討用ラベル）を消すか
⑤ 非表示の橋バリエーションA/C/D（約220行のインラインSVG）を消すか
```
