...
- [営業は3本が並走](project_sales_three_streams.md) — **🔴9/7 社内で実際に動いている管理シート `1IVoOFNk…` が在る（KPI_v2.0.xlsxとは別物・引き継ぎ書に載っていない）。つなげる起点はこれ。設計の前に実物を読む**／★9/7回答＝**①新リストへ書きAIが台帳へ紐付ける(承認)。ただし顧客管理スプレッドへ「どう渡すか」は未設計＝自動共有が要件／②法人単位と事業所単位を両方使う＝数字の正が未決・列が7ずれる・重複コール2,273行／③見本は読み取り専用へ一本化／④数値管理はClaude Code側へ**
