...
- [営業は3本が並走](project_sales_three_streams.md) — **🔴9/7 管理シート `1IVoOFNk…`（ClaudeAIと作成・★まだ記録は入っていない／これで管理しGASを組む前提は決定済み）。つなげる起点はこれ。★「動いています」を「データが入っている」と読み違えた**／**✅9/7確定＝事業所単位から架電／ステータス12語＋★プルダウン化（実装要）／台帳反映は最低翌営業日／中村氏へは数値管理の確定後。★口頭で伝わっているものが多く、案内文は「書き残す」役割**／★9/7回答＝**①新リストへ書きAIが台帳へ紐付ける(承認)。ただし顧客管理スプレッドへ「どう渡すか」は未設計＝自動共有が要件／②法人単位と事業所単位を両方使う＝数字の正が未決・列が7ずれる・重複コール2,273行／③見本は読み取り専用へ一本化／④数値管理はClaude Code側へ**
...
- [営業は3本が並走](project_sales_three_streams.md) — ★9/5決定＝**②テレアポリストを完成させる／③数値管理はステイ(止めるかClaude Code側へ移すかは未確定)**。①台帳紐付けは別途。経路で分ける(福祉テレアポは新型/パートナーは従来通り)＋間にAI精査を1回
